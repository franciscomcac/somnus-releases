...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$HermesHome = $(if ($env:HERMES_HOME) { $env:HERMES_HOME } else { "$env:LOCALAPPDATA\somnus" }),
    [string]$InstallDir = $(if ($env:HERMES_HOME) { "$env:HERMES_HOME\hermes-agent" } else { "$env:LOCALAPPDATA\somnus\hermes-agent" }),
...
$RepoUrlSsh = "git@github.com:franciscomcac/somnus-releases.git"
$RepoUrlHttps = "https://github.com/franciscomcac/somnus-releases.git"
$PythonVersion = "3.11"
...
function Set-ManagedNodeFirstOnUserPath {
    ...
    # Somnus: session-only; never reorder the customer's user PATH.
...
function Install-Git {
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    try {
        ...
        if ($changed) {
            # Somnus: session-only; never add to the customer's user PATH.
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/franciscomcac/somnus-releases/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/franciscomcac/somnus-releases/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/franciscomcac/somnus-releases/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
