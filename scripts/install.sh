...
# Python-driven tool session (e.g. Somnus terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:franciscomcac/somnus-releases.git"
REPO_URL_HTTPS="https://github.com/franciscomcac/somnus-releases.git"
HERMES_HOME="${HERMES_HOME:-$HOME/.somnus}"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Somnus.app)"
            ...
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Somnus installs the code under"
...
emit_manifest() {
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Somnus","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install hermes command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Somnus's own global installs pass an explicit --prefix and are unaffected.
...
install_uv() {
    ...
    # Somnus owns its own uv at $HERMES_HOME/bin/uv.  Always install there —
...
check_python() {
    if [ "$DISTRO" = "termux" ]; then
        ...
        # Somnus currently declares requires-python >=3.11,<3.14.  Termux can
        ...
        log_error "Termux Python $PYTHON_FOUND_VERSION is not supported; Somnus requires Python >=3.11,<3.14"
...
check_node() {
    ...
    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        ...
        log_warn "Node.js $(node --version) is unsupported (Somnus requires Node 22.22+, 24.11+, or 26+) — installing Hermes-managed Node $NODE_VERSION..."
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Somnus install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Somnus itself does not require or retain root access."
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Somnus behaves unexpectedly."
                    else
...
setup_path() {
    ...
    if [ ! -x "$HERMES_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$HERMES_ENTRYPOINT" ]; }; then
        log_warn "Somnus launcher prerequisites not found"
...
EOF
    ...
    # this launcher those hosts report Somnus as not installed. (#21454 applies
...
EOF
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        PATH_COMMENT='# Somnus — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Somnus — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Somnus — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Somnus browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Somnus will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Somnus browser tools — explicit browser override."
...
install_browser_use_cli() {
    ...
    # UV_TOOL_BIN_DIR keeps the binary inside Somnus' managed bin dir, where
...
install_computer_use_driver() {
    ...
    # so a stalled GitHub download can't hang the Somnus install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching Somnus'
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Somnus to send/receive messages."
...
write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.hermes-bootstrap-complete, which tells the Somnus
...
# extract a tree MISSING the electron binary, so the `electron`->`Somnus` rename
