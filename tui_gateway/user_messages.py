...
_TURN_ERROR_LAYER_COPY: dict[str, tuple[str, str]] = {
    ...
    "disk": ("The disk is full, so Somnus could not save the turn", "Free some space, then /retry."),
    "gateway": ("Somnus hit an internal error while running this turn", "Send /retry; type /logs for the trace."),
...
def busy_message(command: str) -> str:
    ...
    return (f"session busy — Somnus is still replying. Stop the current reply first (Stop button, "
            f"or Ctrl+C in a terminal), then run /{command.lstrip('/')}.")


def agent_init_failed_message(exc: Any) -> str:
    return (f"Somnus could not start the assistant for this session. Details: {exc}. "
            "Check the model with /model, or open Settings to reconfigure.")


AGENT_STILL_STARTING = (
    "Somnus is still starting this session (loading tools), so this command could not run yet. "
...
AGENT_MISSING_FOR_TURN = (
    "Somnus could not start the assistant for this session, so your message was not run. "
