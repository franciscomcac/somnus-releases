...
def _is_litellm_route(provider_lower: str, base_url: str) -> bool:
    ...
    return (
        _has_litellm_token(provider_lower, ":-_/")
        or _has_litellm_token(base_url_hostname(base_url), ".-")
        # Somnus: the Somnus gateway is a LiteLLM proxy on its own hostname.
        or base_url_hostname(base_url) in _SOMNUS_GATEWAY_HOSTS
    )


_SOMNUS_GATEWAY_HOSTS = frozenset({"gateway-production-c837.up.railway.app"})
