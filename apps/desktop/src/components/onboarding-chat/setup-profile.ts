 ...
 * sees only Somnus and the title `Welcome to Somnus`.
 *
...
export const SETUP_CHAT_TITLE = 'Welcome to Somnus'
...
export function buildFirstTaskRunbook(
  ...
  return [
    `You are Somnus. The user's welcome chat just opened this session so one task can have room to run: ${task.trim()}.`,
...
const pluginRunbook = (root: string) => [
  'THIS IS A PLUGIN JOB: the thing you are building is a piece of the Somnus app itself, and it will appear in the window the user is looking at right now. That is the whole point — do not let it become a script in a folder.',
  ...
  'Never ask them to restart the app, never edit anything outside their plugin folder, and never touch the Somnus install itself. If the plugin errors on load, the app toasts it and keeps running — read the error, fix the file, save again.'
]
