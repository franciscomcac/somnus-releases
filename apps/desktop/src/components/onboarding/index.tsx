...
// Somnus: customers connect exactly one thing, their Somnus key. The multi-
// provider picker below stays in the tree (upstream parity) but is never shown.
export function SomnusKeyPanel({ ctx }: { ctx: OnboardingContext }) {
  const { t } = useI18n()
  ...
  return (
    <div className="grid gap-3">
      <div className="grid gap-1">
        <span className="text-[length:var(--conversation-text-font-size)] font-semibold">{t.onboarding.somnusKeyTitle}</span>
        <p className="text-xs leading-5 text-muted-foreground">{t.onboarding.somnusKeyDesc}</p>
      </div>
      <Input
        autoComplete="off"
        autoFocus
...
export function Picker({ ctx }: { ctx: OnboardingContext }) {
  return <SomnusKeyPanel ctx={ctx} />
}

export function UpstreamProviderPicker({ ctx }: { ctx: OnboardingContext }) {
  const { t } = useI18n()
