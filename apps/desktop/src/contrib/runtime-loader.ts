...
async function readPluginSourceText(file: string): Promise<string> {
  ...
  if (result.truncated) {
    throw new PluginSourceOversizeError(
      "plugin.js exceeds this shell's 512 KiB read limit — update Somnus to load larger plugins"
    )
  }
...
export async function uninstallDiskPlugin(pluginId: string): Promise<{ ok: boolean; error?: string }> {
  ...
  if (!remove) {
    return { ok: false, error: 'this Somnus build cannot remove desktop plugins — delete the folder by hand' }
  }
