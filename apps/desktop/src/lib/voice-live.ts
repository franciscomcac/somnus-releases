...
/**
 * GPT-Live voice chat: the full-duplex voice frontend that DELEGATES to Somnus.
 *
 ...
 * that into an ordinary Somnus turn on the open session and streams the reply
 * back with `session.commentary.append`, which the voice paraphrases aloud.
 * Somnus keeps every capability — model choice, tools, memory, approvals.
 *
...
export interface VoiceLiveHandlers {
  /** GPT-Live asked the backend (Somnus) for help. `context` is the recent
...
export class VoiceLiveSession {
  ...
  /** The delegation currently being answered by Somnus; late results for an
  ...
  /** Quiet progress for the live model ("Somnus is running the tests…"). */
