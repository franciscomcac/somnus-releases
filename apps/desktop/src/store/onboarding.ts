...
export interface DesktopOnboardingState {
   ...
   *  overlay show its "Somnus is ready" screen once even though the app is
...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'Somnus is ready', message: `${provider} connected.` })
}
...
async function completeWithModelConfirm(
  ...
  if (defaults) {
    ...
    try {
      ...
      onFail(error instanceof Error ? error.message : 'Somnus could not save the selected model.')
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but Somnus still cannot resolve a usable provider. ${detail}`
    : 'Connected, but Somnus still cannot resolve a usable provider.'
}
...
export async function refreshOnboarding(ctx: OnboardingContext, stillWanted?: () => boolean) {
  ...
  if (shouldPreserveConfiguredOnFallback(runtime, state)) {
    ...
    notify({
      ...
      message:
        'Somnus could not verify the running backend on startup. Some features may be unavailable until the gateway is reachable.'
    })
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `Somnus still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
// Somnus: the only provider a customer ever configures. Their Somnus key is an
// OpenAI-compatible key for the Somnus gateway, so this is the custom-endpoint
// path with a fixed URL and a cost-aware default model.
export async function saveSomnusKey(key: string, ctx: OnboardingContext) {
  if (!key.trim()) {
    return { ok: false, message: 'Paste your Somnus key first.' }
  }

  return saveOnboardingLocalEndpoint(SOMNUS_GATEWAY_URL, key, ctx, pickSomnusDefaultModel, 'Somnus')
}
...
export async function saveOnboardingLocalEndpoint(
  ...
  apiKey: string,
  ctx: OnboardingContext,
  chooseModel: (models: string[]) => string = models => models[0] ?? '',
  readyLabel = 'Local / custom endpoint'
) {
  ctx = { ...ctx }
  const generation = flowGeneration
  flowProfile = ctx.profile
  ...
  try {
    ...
    model = chooseModel((probe.models ?? []).map(m => m.trim()).filter(Boolean)).trim()
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Somnus still cannot reach ${resolvedUrl}.` }
    }

    notifyReady(readyLabel)
    completeDesktopOnboarding()
    ctx.onCompleted?.()
