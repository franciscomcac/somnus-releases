...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`Somnus gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.hermesDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update Somnus.')
  }
...
export async function ensureActiveGatewayOpen({
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "Somnus gateway is not connected" on the Sessions `+` action (#88880).
