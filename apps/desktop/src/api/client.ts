...
// "Timed out connecting to Somnus backend" that hangs the UI (#48504).
//
...
export const GATEWAY_NOT_CONNECTED_MESSAGE = 'Somnus gateway is not connected'

export class HermesGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Somnus gateway connection closed',
      connectErrorMessage: 'Could not connect to Somnus gateway',
      ...
      onUnhandledRequest: request =>
        console.warn(`[gateway] Somnus has no server-request registry for ${request.method} (${request.id})`),
