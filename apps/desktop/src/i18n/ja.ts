...
export const ja = defineLocale({
  ...
  boot: {
    ready: 'Somnus の準備ができました',
    desktopBootFailedWithMessage: message => `デスクトップの起動に失敗しました: ${message}`,
    steps: {
      connectingGateway: 'ライブデスクトップゲートウェイに接続中',
      loadingSettings: 'Somnus の設定を読み込み中',
      loadingSessions: '最近のセッションを読み込み中',
      retryingRemoteBackend: 'リモート Somnus バックエンドに再接続中…',
      startingDesktopConnection: 'デスクトップ接続を開始中',
      startingHermesDesktop: 'Somnus を起動中…'
    },
    errors: {
      backgroundExited: 'Somnus バックグラウンドプロセスが終了しました。',
      backgroundExitedDuringStartup: '起動中に Somnus バックグラウンドプロセスが終了しました。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage:
      'Somnus バックエンドがこのデスクトップビルドより古く、正常に動作しない場合があります。更新して揃えてください。',
    installMethodUnsupportedTitle: 'サポート対象外のインストール方法',
    updateHermes: 'Somnus を更新',
    updateReadyTitle: '更新の準備ができました',
    updateReadyMessage: count => `${count} 件の新しい変更が利用可能です。`,
    updateReadyMessageUnknown: '新しい更新が利用可能です。',
    seeWhatsNew: '新機能を見る',
    somnusUpdateReadyTitle: 'Somnus のアップデート準備完了',
    somnusUpdateReadyMessage: v => `Somnus ${v} をダウンロードしました。再起動するとアップデートが完了します（数秒で終わります）。`,
    somnusUpdateRestart: '今すぐ再起動',
    mcp: {
    ...
    errors: {
      ...
      methodNotAllowed:
        'デスクトップバックエンドがそのリクエストを拒否しました (405 Method Not Allowed)。Somnus を再起動してください。',
    ...
    native: {
      ...
      inputBody: 'Somnus が応答を待っています。',
      turnDoneTitle: 'Somnus が完了しました',
      turnDoneBody: '',
  ...
  settings: {
    ...
    resetConfirm: 'すべての設定を Somnus のデフォルトに戻しますか？',
    ...
    vault: {
      ...
      otpHint: '2FA を有効にするときにサイトが表示する「セットアップキー」。保存すると Somnus がコードを生成します。',
      ...
      sources: {
        ...
        notInstalled: name =>
          `未検出です。${name} のコマンドラインツールをインストールしてサインインすると、Somnus が自動的に検出します。`,
        disabledDesc: '検出済みですが、Somnus では無効になっています。',
        lockedDesc:
          '検出済み。エージェントがログイン情報を必要とするときにロック解除を求めます。今すぐ解除することもできます。',
        unlockedDesc: 'このセッションでロック解除済み。30分間操作がないか Somnus を閉じると自動的にロックされます。',
        statusLocked: 'ロック中',
    ...
    notifications: {
      ...
      focusedHint: '完了通知は Somnus がバックグラウンドにあるときのみ表示されます。',
      kinds: {
        approval: {
          label: '承認が必要',
        ...
        input: {
          label: '入力が必要',
          description: 'Somnus が質問したか、パスワードやシークレットを必要としています。'
        },
        turnDone: {
          label: '応答完了',
          description: 'Somnus がバックグラウンドのときにターンが完了しました。'
        },
        turnError: {
          label: 'ターン失敗',
        ...
        plugin: {
          label: 'プラグイン通知',
          description: 'Somnus がバックグラウンドの間に、デスクトッププラグインが通知を送信しました。'
        }
      },
      test: 'テスト通知を送信',
      testTitle: 'Somnus',
    ...
    appearance: {
      ...
      colorMode: 'カラーモード',
      colorModeDesc: '固定モードを選ぶか、Somnus をシステム設定に合わせます。',
      ...
      toursTitle: 'ガイドツアー',
      toursDesc:
        '各ステップを強調しながら、Somnus がアプリを案内します。利用開始から30日後に自動でオフになりますが、再びオンにできます。',
      ...
      pet: {
        title: 'ペット',
        intro:
          'アプリ上に浮かぶ petdex のアニメーションマスコットを採用しましょう。ツール実行中は走り、成功すると喜び、エラーでしょんぼりと、Somnus の状態に反応します。',
        restartHint:
          'ペット機能には再起動が必要です。この機能が追加される前に起動したアプリが動作中です。Somnus を終了して再度開き、このページに戻ってください。',
        scaleTitle: 'サイズ',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone:
        'Somnus がローカル時刻のコンテキストを必要とするときに使用します。空欄ならシステムのタイムゾーンを使います。',
      agent: {
        imageInputMode: '画像添付をモデルへ送る方法を制御します。',
        maxTurns: 'Somnus が 1 回の実行を停止するまでのツール呼び出しターン上限です。'
      },
      terminal: {
      ...
      fileReadMaxChars: 'Somnus が 1 回のファイル読み取りで取得できる最大文字数です。',
      approvals: {
        mode: '明示的な承認が必要なコマンドを Somnus がどう扱うかを設定します。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'アプリから Somnus 自身を更新するとき、ローカルのソース変更を保持するか破棄するかを選びます。ターミナル更新では常に確認されます。'
      }
    }),
    about: {
      heading: 'Somnus',
      ...
      bundleOutOfSyncDesc:
        'Somnus ランタイムは更新されましたが、デスクトップアプリ自体は古いビルドのままです。アプリを更新するまで、新しいインターフェース機能(Bot Mode など)は表示されません。下の更新を実行してアプリを再ビルドしてください。それでもこの警告が消えない場合は、最新のデスクトップインストーラーから再インストールしてください。',
      bundleOutOfSyncAction: 'インストーラーを入手',
      bundleSwapPending: '再起動して更新を完了',
      bundleSwapPendingDesc:
        '更新されたアプリはすでにインストール済みです。Somnus を再起動するだけで新しいビルドが読み込まれます。チャットや設定はそのまま保持されます。',
      bundleSwapPendingAction: 'Somnus を再起動',
      updates: '更新',
      checkNow: '今すぐ確認',
      checking: '確認中…',
      seeWhatsNew: '新機能を見る',
      ...
      automaticUpdatesDesc: 'Somnus はバックグラウンドで自動的に更新を確認し、利用可能になったら通知します。',
    ...
    config: {
      ...
      minimizeToTrayDesc:
        'ウィンドウの最小化やメインウィンドウを閉じる操作でシステムトレイ（macOS ではメニューバー）に隠し、Somnus を実行し続けます。終了するにはトレイメニューの「Somnus を終了」または Cmd+Q を使います。初期設定はオフで、このデバイスにのみ適用されます。',
    ...
    hudModifier: {
      ...
      permission:
        'システム設定 → プライバシーとセキュリティ → 入力監視で Somnus を許可し、再試行してください。このジェスチャーはキー入力の記録や画面の撮影を行いません。',
      unavailable:
        'HUD ジェスチャーヘルパーを起動できなかったか、予期せず停止しました。再試行するか Somnus を再起動してください。Somnus 内の既存の HUD ショートカットは引き続き使用できます。',
      missingHelper:
        'この Somnus には HUD ジェスチャーヘルパーが含まれていません。Somnus を更新または再インストールしてから再試行してください。',
      unsupportedSession:
    ...
    screenshot: {
      ...
      enabledDesc:
        'どのアプリからでも左右の Command キーを同時に押すと、最前面のウインドウを撮影し、Somnus の現在の下書きに添付します。自動送信はしません。初期設定はオフで、この Mac にのみ適用されます。機密情報が写る可能性があるため、送信前に添付画像を確認してください。',
      ...
      inputPermission:
        '入力監視の許可により、他のアプリがアクティブな間も両方の Command キーを検出できます。システム設定 → プライバシーとセキュリティ → 入力監視で Somnus を許可し、ここに戻って再試行してください。',
      screenPermission:
        '画面収録の許可により、このショートカットを使ったときに最前面のアプリのウインドウを撮影できます。システム設定 → プライバシーとセキュリティ → 画面収録で Somnus を許可し、ここに戻って再試行してください。macOS に求められた場合は Somnus を再起動してください。',
    ...
    quickEntry: {
      enabledTitle: 'クイック入力',
      enabledDesc:
        'グローバルショートカットで小さな入力欄をどこからでも呼び出し、Somnus を開かずにプロンプトを送信します。',
    ...
    gateway: {
      ...
      intro:
        'Somnus はデフォルトで独自のローカルゲートウェイを起動します。別のマシンや信頼できるプロキシの背後で既に動作している Somnus バックエンドをこのアプリで制御する場合は、リモートゲートウェイを使用してください。ゲートウェイ接続はマシン単位の設定で、プロファイルは接続したゲートウェイから検出されます。',
      ...
      localDesc:
        'ローカルホストでプライベートな Somnus バックエンドを起動します。これがデフォルトで、オフラインでも動作します。',
      ...
      restartingMessage: 'Somnus は保存された設定を使用して再接続します。',
      savedMessage: '次回起動時に保存されます。',
      connectedTo: (baseUrl, version) => `${baseUrl}${version ? ` · Somnus ${version}` : ''} に接続しました`,
      ...
      sshTitle: 'SSH で接続',
      sshDesc:
        'Somnus は SSH 経由でリモート上に起動され、このアプリにトンネルされます。リモート側で何かを起動・公開する必要はありません。ホストへの鍵ベースの SSH アクセスが前提です。',
      ...
      sshReachable: (host, platform) => `接続可能: ${host}（${platform}）— Somnus を検出`,
      sshIncompleteHost: '接続する前に SSH ホストを入力してください。',
      sshErrUnreachable: 'SSH でそのホストに到達できませんでした。ホスト、ポート、ネットワークを確認してください。',
      sshErrAuth:
        'SSH 認証に失敗しました。鍵を ssh-agent に読み込む（ssh-add）か、~/.ssh/config に IdentityFile を設定してください。Somnus は非対話的に ssh を実行します。',
      sshErrHostKey:
      ...
      sshErrUpdateRequired: 'Desktop SSH で接続する前に、リモートホストの Somnus を更新してください。',
    ...
    mcp: {
      ...
      deepLinkDescription:
        'リンクがこの MCP サーバーを Somnus に追加するよう要求しました。下の設定はリンク側から来たものです。内容を必ず確認してください。',
    ...
    localModels: {
      ...
      runtimeInstalledDetail: (tag, backend) =>
        `ビルド ${tag}、${backend} バックエンド。サーバーは Somnus が起動・管理します。`,
    ...
    providers: {
      ...
      intro:
        'サブスクリプションでサインインします。API キーのコピーは不要です。Somnus がアプリ内でブラウザーサインインを代行します。',
      connected: '接続済み',
      collapse: '折りたたむ',
    ...
    toolsets: {
      ...
      browserRealProfile: {
        ...
        prompt: {
          ...
          body: 'Somnus が既定ブラウザプロファイルのスナップショットでブラウジングできるようにすると、サイトはログイン済みの状態で開きます。',
  ...
  starmap: {
    ...
    emptyDesc: 'Somnus がスキルやメモリを蓄積すると、ここに表示されます。'
  },
  agents: {
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: 'ペット機能を使うには Somnus を再起動してください。',
      empty: '一致するペットがありません。',
      turnOff: 'オフ',
      turnOn: 'オン',
      installed: 'インストール済み',
    ...
    generatePet: {
      ...
      staleBackend: 'ペットを生成するには Somnus を更新してください。',
      backgroundHint: 'このウィンドウは閉じても大丈夫です。完了したら Somnus が通知します。',
    ...
    nav: {
      ...
      settings: { title: '設定', detail: 'Somnus デスクトップを設定' },
    ...
    hermesActiveSessions: (version, count) => `Somnus ${version} · アクティブセッション ${count}`,
  ...
  messaging: {
    ...
    telegramQr: {
      ...
      subtitle: 'どちらの方法でも、あなたが管理するボットを接続し、資格情報はこの Somnus にのみ保存されます。',
      quickSetup: 'クイックセットアップ',
      recommended: '推奨',
      quickHelp:
        'QR コードをスキャンして Telegram で確認します。Somnus がボットを作成し、あなたの Telegram ユーザー ID を自動検出します。',
      createWithQr: 'QR で作成',
      starting: '開始中…',
      replaceWarning:
  ...
  profiles: {
    ...
    defaultDescription: 'Somnus の起動時と新しいチャットに使用します。既存のセッションのプロファイルは変わりません。',
    ...
    deleting: '削除中...',
    createDesc: 'プロファイルは独立した Somnus 環境です：設定、スキル、SOUL.md が別々になります。',
    nameLabel: '名前',
    cloneFrom: '複製元',
    cloneFromNone: 'なし（空）',
  ...
  modelAssignment: {
    saveFailed: 'Somnus はモデルの変更を保存しませんでした。',
  ...
  cron: {
    ...
    newCron: '新しい Cron',
    emptyDescNew:
      'Cron 式でプロンプトを実行するスケジュールを設定します。Somnus が実行して、選択した宛先に結果を送信します。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      staleBackend:
        'プロジェクトを作成するには Somnus バックエンドを更新してください。バックエンドがこのデスクトップアプリより古いです（設定 → 更新 → バックエンド）。',
      deleteConfirm:
        'Somnus から保存済みプロジェクトを削除します。ファイル・git リポジトリ・ワークツリーはそのまま残ります。',
      startWork: '新しいワークツリー',
      ...
      worktreeStaleBackend:
        'このリモート接続でワークツリーを作成するには Somnus バックエンドを更新してください — git ワークツリー API 以前のバージョンです。',
  ...
  composer: {
    ...
    placeholderStarting: 'Somnus を起動中...',
    placeholderReconnecting: 'Somnus に再接続中…',
    ...
    attachUrlDesc: 'Somnus がページを取得し、このターンのコンテキストとして含めます。',
  ...
  statusStack: {
    ...
    coding: {
      ...
      agentShip: 'Somnus にコミットと PR を任せる',
  ...
  updates: {
    ...
    somnusAutoTitle: 'Somnus は自動でアップデートされます',
    somnusAutoMessage: 'Somnus は常に最新の状態に保たれます。新しいバージョンはバックグラウンドでダウンロードされ、次回の再起動時にインストールされます。',
    unsupportedMessage: 'このバージョンの Somnus はアプリ内から自分を更新できません。',
    ...
    availableBody: '新しいバージョンの Somnus をインストールする準備ができています。',
    availableTitleBackend: 'バックエンドの更新があります',
    availableBodyBackend: '接続中の Somnus バックエンドの新しいバージョンをインストールできます。',
    ...
    manualBody:
      'Somnus をコマンドラインからインストールしたため、更新もそこで実行されます。これをターミナルに貼り付けてください:',
    manualPickedUp: 'Somnus は次回起動時に新しいバージョンを読み込みます。',
    guiSkewTitle: 'デスクトップアプリを更新してください',
    guiSkewBody:
      'バックエンドは更新されましたが、このデスクトップアプリのパッケージは変更されていません。一致させるために Somnus デスクトップアプリ（AppImage / .deb / .rpm）を更新または再インストールしてください。',
    copy: 'コピー',
    copied: 'コピーしました',
    done: '完了',
    applyingBody:
      'Somnus アップデーターが独自のウィンドウで引き継ぎ、完了後に自動的に Somnus を再度開きます。更新中はご自分で Somnus を開き直さないでください。',
    applyingBodyBackend: 'リモートバックエンドが更新を適用して再起動します。復帰すると Somnus が自動的に再接続します。',
    applyingClose: 'このウィンドウは更新中に閉じ、その後 Somnus が自動的に再度開きます。',
    errorTitle: '更新が完了しませんでした',
    errorBody: 'ご安心ください。何も失われていません。今すぐ再試行できます。',
    blockerTitle: 'Somnus を更新するためにローカルプレビューを閉じますか？',
    blockerBody:
      '更新する前に、これらのローカルプレビューを停止する必要があります。ファイルが変更または削除されることはありません。',
    foreignBlockerTitle: '他のプロセスを閉じて Somnus を更新',
    foreignBlockerBody:
      'Somnus はこれらのプロセスを安全に自動終了できません。各プロセスを所有するアプリ、ターミナル、またはサービスを閉じてから、もう一度更新してください。',
    mixedBlockerBody:
      'Somnus は以下のローカルプレビューを閉じることができます。更新を続けるには、他のプロセスを手動で閉じる必要があります。',
  ...
  guidedGreeting: {
    line: 'やあ、どうぞ。Somnus です。二分だけください、あなたに合わせて整えます。それから、本当にやりたいことに取りかかりましょう。\n\nまずは、何とお呼びすればいいですか。',
  ...
  install: {
    ...
    oneTimeTitle: 'Somnus には一度限りのインストールが必要です',
    ...
    setupChoiceTitle: 'Somnus をセットアップ',
    setupChoiceDesc:
      'すでに実行している Somnus ゲートウェイに接続するか、このコンピューターに Somnus をローカルインストールします。',
    connectExistingTitle: '既存の Somnus に接続',
    connectExistingShort: '既存環境に接続',
    connectExistingDesc:
      'セッショントークンまたはブラウザーサインインでリモートバックエンドを使用します。ローカルインストールは開始されません。',
    installLocalTitle: 'Somnus をローカルにインストール',
    installLocalDesc: 'Somnus をダウンロードし、Python 環境を作成して、このコンピューターでバックエンドを実行します。',
    localStartUnavailable:
      'ローカルインストールを開始できません。Somnus を再起動して、もう一度お試しください。',
    remoteSetupTitle: '既存の Somnus に接続',
    remoteSetupDesc:
    ...
    settingUpTitle: 'Somnus を設定中',
    finishingTitle: '仕上げ中',
    failedDesc:
      'インストール手順のいずれかが失敗しました。Windows では、別の Somnus CLI またはデスクトップインスタンスが実行中の場合に発生することがあります。実行中の Somnus インスタンスをすべて停止してから再試行してください。詳細は以下またはデスクトップログで確認できます。',
    activeDesc:
      'これは一回限りのセットアップです。Somnus インストーラーが依存関係をダウンロードしてマシンを設定しています。以降の起動ではこの手順はスキップされます。',
  ...
  onboarding: {
    ...
    somnusSignInDesc: 'ブラウザでサインインするか、無料アカウントを作成してください。完了すると Somnus が自動的に接続します。',
    somnusSignInWaiting: 'ブラウザを待っています…',
    somnusUseKey: 'キーを使う',
    somnusSignInFailed: 'サインインが完了しませんでした。もう一度お試しください。',
    somnusAccountNav: 'Somnus アカウント',
    somnusAccountTitle: 'Somnus アカウント',
    somnusAccountDesc: 'すべてのモデルは Somnus アカウントで動作し、Somnus の残高から請求されます。チャージや利用状況の確認は Somnus のウェブサイトで行えます。',
    ...
    somnusAccountUsage30d: (spend, requests) => `過去30日間の利用額 ${spend} · ${requests} リクエスト`,
    somnusAccountTopUp: 'チャージ',
    somnusAccountLowCredit: 'クレジットがありません。Somnus を使い続けるにはチャージしてください。',
    somnusAccountSignOut: 'サインアウト',
    somnusAccountSignOutTitle: 'Somnus からサインアウトしますか？',
    somnusAccountSignOutDesc: '再度サインインするまで、このコンピューターの Somnus はあなたのアカウントを使用しません。チャットと設定はそのまま残り、クレジットもアカウントに残ります。',
    somnusAccountSignedOut: 'Somnus からサインアウトしました',
    somnusAccountNotSignedIn: 'このコンピューターの Somnus はサインインしていません。',
    somnusAccountExpired: 'サインインの有効期限が切れました。再度サインインしてください。',
    somnusAccountUnavailable: '現在、残高を読み込めません。',
    ...
    recommended: '推奨',
    connected: '接続済み',
    featuredPitch: '1 つのサブスクリプションで 300 以上の最先端モデル — Somnus を実行するための推奨方法',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: 'セルフホスト',
        description:
          'ローカルまたはセルフホストの OpenAI 互換エンドポイント（vLLM、llama.cpp、Ollama など）に Somnus を接続。'
      }
    },
    ...
    flowSubtitles: {
      ...
      device_code: 'ブラウザーで確認ページを開きます — Somnus が自動接続します',
    ...
    authorizeThere: 'そこで Somnus を承認してください。',
    copyAuthCode: '認証コードをコピーして以下に貼り付けてください。',
    pasteAuthCode: '認証コードを貼り付け',
    reopenAuthPage: '認証ページを再度開く',
    autoBrowser: provider =>
      `${provider} をブラウザーで開きました。Somnus をそこで承認すれば自動接続されます。コピーや貼り付けは不要です。`,
  ...
  preview: {
    ...
    largeBody: (label, size) => `${label} は ${size} です。Somnus は最初の 512 KB のみを表示します。`,
    ...
    web: {
      ...
      tryAgain: '再試行',
      restarting: 'Somnus を再起動中...',
      askRestart: 'Somnus にサーバーの再起動を依頼',
      lookingRestart: taskId => `Somnus は再起動するプレビューサーバーを検索中です (${taskId})`,
      restartingTitle: 'プレビューサーバーを再起動中',
      restartingMessage: 'Somnus はバックグラウンドで作業中です。進捗はプレビューコンソールで確認してください。',
      ...
      blankPageBody: '上のアドレス欄に入力するか、Somnus にページを開くよう頼んでください。',
      finishedRestarting: message =>
        `Somnus がプレビューサーバーの再起動を完了しました${message ? `: ${message}` : ''}`,
      ...
      restartFailedMessage: 'Somnus がサーバーを再起動できませんでした。',
      stillWorking:
        'Somnus はまだ作業中ですが、再起動の結果がまだ届いていません。サーバーコマンドがフォアグラウンドで実行されている可能性があります。',
  ...
  interfaceMode: {
    title: 'インターフェースモード',
    hint: '表示される内容が変わるだけで、Somnus にできることは変わりません。',
    sessionNote:
      'シンプルモードで設定されています。ここでの変更はこのセッション中のみ有効です。自分の設定にするには詳細モードに切り替えてください。',
    simple: {
      label: 'シンプル',
      description:
        'Somnus と話すための表示。サイドバーとチャットのみ。ターミナル、ファイル、差分のペインは表示しません。'
    },
    advanced: {
      label: '詳細',
  ...
  assistant: {
    thread: {
      ...
      loadingResponse: 'Somnus が応答を読み込み中',
    ...
    approval: {
      gatewayDisconnected: 'Somnus ゲートウェイが接続されていません',
  ...
  prompts: {
    ...
    secretDesc: 'Somnus は続行するための認証情報が必要です。',
    ...
    vaultSaveDesc: origin =>
      `Somnus は ${origin} のサインインページに到達しましたが、保存されたログイン情報がありません。ここで一度入力すると、このマシン上で暗号化して保存され、ページに直接入力されます。モデルはパスワードを一切見ません。`,
    ...
    vaultCodeDesc: site =>
      `${site} がワンタイムコード（SMS、メール、または認証アプリ）を求めています。ここに入力すると Somnus がページに入力します。モデルはコードを一切見ません。`,
    vaultCodeLabel: 'コード',
    vaultCodeFootnote:
      'ヒント：「設定 → パスワードとログイン」でこのログインに認証キーを保存すると、Somnus がコードを自動入力します。',
    vaultCodeSkip: 'スキップ',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '画像を保存するには Somnus を再起動してください。',
    restartToSaveImages: '画像を保存するには Somnus を再起動してください',
  ...
  tips: {
    ...
    items: {
      ...
      skills: {
        title: '一度教えれば覚えます',
        text: 'スキルは手順書のフォルダで、必要な場面で Somnus が自分で読み込みます。'
      },
      messaging: {
        title: 'デスクを離れても Somnus',
        text: 'Telegram、Discord、Slack などに接続。同じエージェント、同じ記憶のままです。'
      },
      artifacts: {
        title: 'Somnus が作ったものすべて',
      ...
      profiles: {
        ...
        text: 'それぞれが独自のキー・メモリ・セッションを持つ、別の Somnus です。'
      },
      'composer-mentions': {
        title: 'ファイルとコマンド',
