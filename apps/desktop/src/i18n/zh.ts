...
export const zh = defineLocale({
  connectors: {
    ...
    search: '查找应用',
    empty: '没有匹配的应用',
    disclaimer: '连接为可选操作。请仅授权你希望 Somnus 使用的应用。',
    execution: '连接器工具',
  ...
  sessionImport: {
    title: '从其他应用继续',
    subtitle: '将对话导入 Somnus，接着上次的进度继续。',
    action: '导入会话',
    readingFrom: '读取自',
  ...
  boot: {
    ...
    steps: {
      ...
      loadingSettings: '正在加载 Somnus 设置',
      loadingSessions: '正在加载最近会话',
      retryingRemoteBackend: '正在重新连接远程 Somnus 后端…',
      startingDesktopConnection: '正在启动桌面连接',
      startingHermesDesktop: '正在启动 Somnus 桌面版…'
    },
    errors: {
      backgroundExited: 'Somnus 后台进程已退出。',
      backgroundExitedDuringStartup: 'Somnus 后台进程在启动期间退出。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage: '你的 Somnus 后端早于当前桌面构建，可能无法正常工作。请更新以保持一致。',
    installMethodUnsupportedTitle: '不受支持的安装方式',
    updateHermes: '更新 Somnus',
    updateReadyTitle: '有可用更新',
    updateReadyMessage: count => `有 ${count} 项新更改可用。`,
    updateReadyMessageUnknown: '有新更新可用。',
    seeWhatsNew: '查看更新内容',
    somnusUpdateReadyTitle: 'Somnus 更新已就绪',
    somnusUpdateReadyMessage: v => `Somnus ${v} 已下载。重启即可完成更新，只需几秒钟。`,
    somnusUpdateRestart: '立即重启',
    mcp: {
    ...
    errors: {
      ...
      methodNotAllowed: '桌面后端拒绝了该请求 (405 Method Not Allowed)。请尝试重启 Somnus。',
    ...
    voice: {
      ...
      liveError: '实时语音',
      liveDelegationFailed: '无法将请求交给 Somnus',
    ...
    native: {
      ...
      inputBody: 'Somnus 正在等待你的回应。',
      turnDoneTitle: 'Somnus 已完成',
      turnDoneBody: '',
      turnErrorTitle: '本轮失败',
  ...
  settings: {
    ...
    resetConfirm: '将所有设置恢复为 Somnus 默认值？',
    exportFailed: '导出失败',
    resetFailed: '重置失败',
    nav: {
      providers: '提供方',
    ...
    vault: {
      ...
      otpHint: '启用两步验证时网站显示的“设置密钥”。保存后 Somnus 会自动生成验证码。',
      ...
      sources: {
        ...
        notInstalled: name => `未检测到。安装 ${name} 命令行工具并登录后，Somnus 会自动识别。`,
        disabledDesc: '已检测到，但已为 Somnus 关闭。',
        lockedDesc: '已检测到。智能体需要登录信息时会请你解锁，也可立即解锁。',
        unlockedDesc: '本会话已解锁。闲置 30 分钟或关闭 Somnus 后会自动锁定。',
        statusLocked: '已锁定',
    ...
    plugins: {
      title: '桌面插件',
      blurb:
        '加载到此应用中的界面扩展——随构建捆绑，或放入 desktop-plugins 文件夹（包括 Somnus 编写的插件）。禁用会即时卸载插件并在重启后保持。',
      count: n => `已安装 ${n} 个`,
      ...
      installModal: {
        ...
        catalogPinned: (name, sha) =>
          `Somnus 目录条目「${name}」— agent 部分将安装在经过审核的固定提交${sha ? ` ${sha}` : ''}，而不是分支最新代码。`,
    ...
    notifications: {
      ...
      focusedHint: '完成提醒仅在 Somnus 处于后台时触发。',
      kinds: {
        approval: {
          label: '需要批准',
        ...
        input: {
          label: '需要输入',
          description: 'Somnus 提出了问题，或需要密码或密钥。'
        },
        turnDone: {
          label: '回复就绪',
          description: 'Somnus 在后台时完成了一轮对话。'
        },
        turnError: {
          label: '本轮失败',
          description: '后台回合错误。'
        ...
        plugin: {
          label: '插件通知',
          description: 'Somnus 在后台时，桌面插件发送了通知。'
        }
      },
      test: '发送测试通知',
      testTitle: 'Somnus',
    ...
    appearance: {
      ...
      colorMode: '颜色模式',
      colorModeDesc: '选择固定模式，或让 Somnus 跟随系统设置。',
      ...
      reactionsTitle: '消息回应',
      reactionsDesc: 'iMessage 风格的表情回应 — 你可以给消息添加回应，Somnus 也能回应你的消息。',
      tipsTitle: '应用内提示',
      tipsDesc: '偶尔显示来自应用和 Somnus 的提示，每条提示只出现一次。开始使用满30天后自动关闭，你可以重新开启。',
      tipsReset: (count: number) => `再次显示 ${count} 条提示`,
      toursTitle: '引导导览',
      toursDesc: '让 Somnus 逐步高亮每个位置，带你熟悉应用。开始使用满30天后自动关闭，你可以重新开启。',
      ...
      pet: {
        title: '宠物',
        intro:
          '领养一只悬浮在应用上的 petdex 动画宠物，它会根据 Somnus 的状态做出反应——工具执行时奔跑、成功时欢呼、出错时沮丧。',
        restartHint: '宠物功能需要重启——当前运行的应用在此功能加入前启动。请退出并重新打开 Somnus，然后回到此处。',
        scaleTitle: '大小',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone: '当 Somnus 需要本地时间上下文时使用。留空则使用系统时区。',
      agent: {
        imageInputMode: '控制图片附件如何发送给模型。',
        maxTurns: 'Somnus 停止一次运行前工具调用轮次的上限。'
      },
      terminal: {
        cwd: '工具与终端操作的默认项目目录。',
      ...
      fileReadMaxChars: 'Somnus 单次文件读取可读取的最大字符数。',
      approvals: {
        mode: 'Somnus 如何处理需要显式审批的命令。',
      ...
      browser: {
        useRealProfile:
          '本地浏览使用你的真实登录状态。Somnus 会把你默认浏览器的配置（Cookie、登录、偏好）复制为受管快照，并用自带的 Chromium 驱动它——不会直接打开你的实时配置，且每次运行都会从实时配置刷新副本。还允许智能体在配置了云端浏览器后端时，按需打开本地真实配置会话。仅支持 Chromium 系浏览器（Chrome、Edge、Brave、Brave Origin、Chromium）；默认浏览器不是 Chromium 系时会给出明确报错。默认关闭。'
      },
      voice: {
        autoTts: '自动朗读助手回复。',
        voiceChatMode:
          'chained：语音转文字 → Somnus → 文字转语音，使用下方的提供商。gpt-live：一个全双工的 OpenAI 语音模型（gpt-live-1）负责听和说，并把每个实际请求交给 Somnus——由你选择的任意模型带着完整工具集作答。需要 OpenAI API 密钥；语音层按每分钟 $0.05 计费。',
        gptLive: {
          voice: 'GPT-Live 模式使用的音色，可填写自定义音色 ID。',
          instructions: '附加到实时语音人设的句子（语气、语速、语言）。Somnus 保留自己的系统提示词。'
        }
      },
      stt: {
      ...
      updates: {
        nonInteractiveLocalChanges:
          'Somnus 从应用内更新时（无终端提示），保留本地源码修改（暂存）或丢弃（放弃）。通过终端更新时始终会询问。'
      }
    }),
    uninstallSection: {
      dangerZone: '危险操作',
      confirmUninstall: '确认卸载',
      uninstallHermes: '卸载 Somnus'
    },
    poolLimits: {
    ...
    about: {
      ...
      bundleOutOfSyncDesc:
        'Somnus 运行时已更新,但桌面应用本身仍是旧构建——在应用更新之前,新的界面功能(如 Bot Mode)不会显示。请运行下方的更新以重新构建应用。如果此警告仍未消除,请从最新的桌面安装程序重新安装。',
      bundleOutOfSyncAction: '获取安装程序',
      bundleSwapPending: '重启以完成更新',
      bundleSwapPendingDesc: '更新后的应用已安装完成，只需重启 Somnus 即可加载新版本。聊天记录和设置不会受到影响。',
      bundleSwapPendingAction: '重启 Somnus',
      updates: '更新',
      checkNow: '立即检查',
      checking: '检查中…',
      seeWhatsNew: '查看新增内容',
      ...
      automaticUpdatesDesc: 'Somnus 会在后台自动检查更新，并在有可用更新时通知你。',
    ...
    config: {
      ...
      minimizeToTrayDesc:
        '最小化窗口或关闭主窗口时，将其隐藏到系统托盘（macOS 上为菜单栏），让 Somnus 继续运行。通过托盘菜单中的“退出 Somnus”或 Cmd+Q 退出。默认关闭，仅适用于此设备。',
    ...
    hudModifier: {
      ...
      permission: '请在系统设置 → 隐私与安全性 → 输入监控中允许 Somnus，然后重试。此手势不会记录按键或截取屏幕。',
      unavailable: 'HUD 手势辅助程序无法启动或意外停止。请重试或重启 Somnus。Somnus 内原有的 HUD 快捷键仍可使用。',
      missingHelper: '此 Somnus 安装缺少 HUD 手势辅助程序。请更新或重新安装 Somnus，然后重试。',
      unsupportedSession: '此桌面会话不支持全局修饰键轻按事件。Linux 需要 X11；不支持 Wayland。'
    },
    screenshot: {
      enabledTitle: '截图快捷键',
      enabledDesc:
        '在任意应用中同时按下左右两个 Command 键，即可截取最前面的窗口并附加到当前 Somnus 草稿。绝不会自动发送。默认关闭，仅适用于这台 Mac。窗口可能包含敏感内容，请在发送前检查附件。',
      statusTitle: '截图快捷键状态',
      checking: '正在检查截图快捷键…',
      disabled: '截图快捷键已关闭。',
      ...
      inputPermission:
        '输入监控权限允许 Somnus 在其他应用处于活动状态时检测两个 Command 键。请在系统设置 → 隐私与安全性 → 输入监控中允许 Somnus，然后返回此处重试。',
      screenPermission:
        '屏幕录制权限允许 Somnus 在你使用此快捷键时截取最前面的应用窗口。请在系统设置 → 隐私与安全性 → 屏幕录制中允许 Somnus，然后返回此处重试。如果 macOS 提示，请重启 Somnus。',
      openSettings: '打开系统设置',
      retry: '重试',
    ...
    quickEntry: {
      enabledTitle: '快速输入',
      enabledDesc: '用全局快捷键在任何地方唤出一个小输入框，无需打开 Somnus 即可发送提示。',
    ...
    connections: {
      title: '已注册网关',
      intro: '管理本机以及通过远程、SSH 或 Somnus Cloud 连接可访问的每个 Somnus 网关。',
      stagedNote:
      ...
      updateSkippedCloud: '由 Somnus Cloud 托管',
      kindLocal: '本地',
      kindRemote: '远程网关',
      kindCloud: 'Somnus Cloud',
      kindSsh: 'SSH',
      kindLocalDesc: '由本应用管理的 Somnus 运行时。',
      kindRemoteDesc: '可通过 HTTP(S) 访问的 Somnus 网关——局域网、Tailscale 或互联网。',
      kindCloudDesc: '通过你的 Somnus Cloud 账户发现的托管实例。',
      kindSshDesc: '通过 SSH 访问的 Somnus 安装。',
      labelTitle: '名称',
      ...
      headersDesc:
        '随发往此网关的每个 HTTP 和 WebSocket 请求一起发送——用于 Cloudflare Access 等访问代理（CF-Access-Client-Id / CF-Access-Client-Secret）。值加密存储。由 Somnus 管理的请求头（Authorization、Cookie、Host 等）会被忽略。',
      ...
      cloudAddHint: '提示：在上方登录 Somnus Cloud 可自动发现你的智能体——此表单仅用于手动注册已知的实例 URL。',
      save: '保存连接',
      saving: '保存中…',
      cancel: '取消',
      empty: '尚未注册任何连接。'
    ...
    gateway: {
      ...
      envOverride: '环境变量覆盖',
      intro:
        'Somnus 默认会启动自己的本地网关。当你希望此应用控制另一台机器上或可信代理后的现有 Somnus 后端时，可以使用远程网关。网关连接属于本机级设置；profile 是从所连接的网关中发现的。',
      envOverrideTitle: '环境变量正在控制此桌面会话。',
      envOverrideDesc: '取消设置 HERMES_DESKTOP_REMOTE_URL 和 HERMES_DESKTOP_REMOTE_TOKEN 后才会使用下面保存的设置。',
      modeTitle: '连接模式',
      localTitle: '本地网关',
      localDesc: '在 localhost 启动私有 Somnus 后端。这是默认方式，并且可离线工作。',
      remoteTitle: '远程网关',
      remoteDesc: '将此桌面外壳连接到远程 Somnus 后端。',
      remoteAuthHint: '托管网关使用 OAuth 或用户名密码；自托管网关也可能使用会话 token。',
      cloudTitle: 'Somnus Cloud',
      cloudDesc: '只需登录 Somnus Cloud 一次，即可从你账户下的智能体中选择——无需粘贴 URL。',
      cloudSignInTitle: 'Somnus Cloud',
      cloudSignIn: '登录 Somnus Cloud',
      cloudSignedIn: '已登录 Somnus Cloud',
      cloudNeedsSignIn: '登录 Somnus Cloud 以发现你账户下的智能体。',
      ...
      cloudDiscoverFailed: '无法加载你的 Somnus Cloud 智能体',
      cloudConnectFailed: '无法连接到该智能体',
      cloudSignInFailed: 'Somnus Cloud 登录失败',
      cloudSignedOutTitle: '已退出 Somnus Cloud',
      cloudSignedOutMessage: '已清除 Somnus Cloud 会话。',
      ...
      savedTitle: '网关设置已保存',
      restartingMessage: 'Somnus 将使用已保存设置重新连接（界面保持打开）。',
      savedMessage: '已保存，下一次重启生效。',
      connectedTo: (baseUrl, version) => `已连接到 ${baseUrl}${version ? ` · Somnus ${version}` : ''}`,
      ...
      sshTitle: '通过 SSH 连接',
      sshDesc:
        'Somnus 会通过 SSH 在远程启动并以隧道连接到本应用——无需自行启动或暴露任何服务。前提：已具备到该主机的密钥 SSH 访问。',
      ...
      sshReachable: (host, platform) => `可连接：${host}（${platform}）——已找到 Somnus`,
    ...
    mcp: {
      ...
      deepLinkDescription:
        '一个链接请求将此 MCP 服务器添加到 Somnus。请检查下方的完整配置——它来自该链接，而非 Somnus。',
    ...
    localModels: {
      ...
      runtimeInstalledDetail: (tag, backend) => `构建 ${tag}，${backend} 后端。Somnus 会为您启动并管理服务器。`,
    ...
    providers: {
      ...
      intro: '使用订阅登录，无需复制 API 密钥。Somnus 会在应用中为你完成浏览器登录。',
      connected: '已连接',
      collapse: '收起',
      ...
      localEndpoint: {
        title: '本地 / 自定义端点',
        description: '将 Somnus 指向任意 OpenAI 兼容端点（Zyphra、vLLM、llama.cpp、Ollama 等）。'
      },
      loading: '正在加载提供方...'
    },
    sessions: {
    ...
    toolsets: {
      ...
      browserRealProfile: {
        ...
        prompt: {
          title: '让网站保持登录状态',
          body: '让 Somnus 使用默认浏览器配置文件的快照进行浏览，网站打开时即已登录。',
  ...
  skills: {
    ...
    plugins: {
      ...
      legacyBackend: '此后端版本较旧，不支持按键名切换插件 — 请更新 Somnus 后再在此管理。',
      portableBadge: '便携',
      catalogTitle: '插件目录',
      catalogBrowse: '浏览',
      catalogHide: '隐藏目录浏览器',
      ...
      catalogProvenance: (sha: string) => `从 Somnus 目录安装${sha ? `，固定提交 ${sha}` : ''}。`,
      ...
      deepLinkCatalogUnknown: (name: string) => `“${name}”不在 Somnus 插件目录中。未安装任何内容。`,
      deepLinkCatalogUnavailable: '无法加载 Somnus 插件目录。请检查网络连接后重新打开链接。'
    },
    officialCatalog: '可安装',
    officialPill: '官方',
    hub: {
  ...
  starmap: {
    ...
    loading: '加载中…',
    emptyTitle: '尚无学习内容',
    emptyDesc: '当 Somnus 为你的工作构建技能和记忆时，会显示在这里。',
    share: '分享图谱',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: '请重启 Somnus 以使用宠物功能——当前后端版本过旧。',
      empty: '没有匹配的宠物。',
      turnOff: '关闭',
      turnOn: '开启',
      installed: '已安装',
    ...
    generatePet: {
      ...
      staleBackend: '请更新 Somnus 以生成宠物。',
      backgroundHint: '你可以关闭此窗口——完成后 Somnus 会通知你。',
    ...
    nav: {
      ...
      settings: { title: '设置', detail: '配置 Somnus 桌面端' },
    ...
    hermesActiveSessions: (version, count) => `Somnus ${version} · 活跃会话 ${count}`,
    restartGateway: '重启网关',
    openBrowser: '打开浏览器',
  ...
  messaging: {
    ...
    telegramQr: {
      ...
      subtitle: '两种方式都会连接一个由你控制的机器人，凭据仅保存在此 Somnus 安装中。',
      quickSetup: '快速设置',
      recommended: '推荐',
      quickHelp: '扫描二维码并在 Telegram 中确认。Somnus 会自动创建机器人并识别你的 Telegram 用户 ID。',
    ...
    platformIntro: {
      ...
      signal: '在可访问的位置运行 signal-cli REST 桥接，然后把 Somnus 指向该 URL 和已注册的电话号码。',
      whatsapp: '启动 Somnus 自带的 WhatsApp 桥接，首次运行时扫描二维码，然后启用该平台。',
      bluebubbles:
        '在装有 iMessage 的 Mac 上运行 BlueBubbles Server，暴露其 API，然后用服务器密码把 Somnus 指向该 URL。',
  ...
  profiles: {
    ...
    defaultDescription: '用于 Somnus 启动和新建聊天。现有会话仍保留在各自的配置档案中。',
    ...
    deleting: '删除中…',
    createDesc: '配置档案是相互独立的 Somnus 环境：各自拥有独立的配置、技能和 SOUL.md。',
    nameLabel: '名称',
    cloneFrom: '克隆来源',
    cloneFromNone: '无（空白）',
  ...
  cron: {
    ...
    newCron: '新建定时任务',
    emptyDescNew: '按 cron 表达式排程一个提示词。Somnus 会运行它，并把结果发送到你选择的目的地。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      createFailed: '无法创建项目',
      staleBackend: '请更新 Somnus 后端以创建项目——当前后端比桌面应用旧（设置 → 更新 → 后端）。',
      deleteConfirm: '这会从 Somnus 中移除已保存的项目。文件、git 仓库和工作树保持不变。',
      startWork: '新建工作树',
      ...
      worktreeStaleBackend: '请更新 Somnus 后端以在此远程连接上创建工作树 —— 该后端早于 git 工作树 API。',
  ...
  composer: {
    ...
    placeholderStarting: '正在启动 Somnus…',
    placeholderReconnecting: '正在重新连接 Somnus…',
    ...
    voiceControls: '语音',
    voiceEngine: '语音聊天引擎',
    voiceEngineChained: '语音转文字 + Somnus 语音',
    voiceEngineLive: 'GPT-Live（全双工，委托给 Somnus）',
    ...
    attachUrlDesc: 'Somnus 将抓取该页面并作为本回合的上下文。',
  ...
  updates: {
    ...
    somnusAutoTitle: 'Somnus 会自动更新',
    somnusAutoMessage: 'Somnus 会自动保持最新。新版本会在后台下载，并在下次重启 Somnus 时安装。',
    unsupportedMessage: '此版本的 Somnus 无法在应用内自行更新。',
    connectionRetry: '请检查网络连接后重试。',
    gitUnusable: 'Somnus 无法在这台电脑上运行 Git，因此无法检查更新。',
    latestBody: '你正在运行最新版本。',
    latestBodyBackend: '后端正在运行最新版本。',
    allSetTitle: '已是最新',
    availableTitle: '有可用更新',
    availableBody: '新版 Somnus 已可安装。',
    availableTitleBackend: '后端有可用更新',
    availableBodyBackend: '已连接的 Somnus 后端有新版本可安装。',
    availableBodyNoChangelog: '已有新版本可用。此安装方式无法显示更新日志。',
    updateNow: '立即更新',
    maybeLater: '稍后再说',
    moreChanges: count => `另有 ${count} 项更改。`,
    manualTitle: '从终端更新',
    manualBody: '你是从命令行安装的 Somnus，因此更新也需要在那里运行。请将此命令粘贴到终端：',
    manualPickedUp: '下次启动 Somnus 时会使用新版本。',
    guiSkewTitle: '请更新桌面应用',
    guiSkewBody:
      '后端已更新，但此桌面应用包未更改。请更新或重新安装 Somnus 桌面应用（你的 AppImage / .deb / .rpm）以保持一致。',
    copy: '复制',
    copied: '已复制',
    done: '完成',
    applyingBody:
      'Somnus 更新器会在自己的窗口中接管，并在完成后自动重新打开 Somnus。更新期间请不要自行重新打开 Somnus。',
    applyingBodyBackend: '远程后端正在应用更新并将重启。恢复后 Somnus 会自动重新连接。',
    applyingClose: '此窗口会在更新期间关闭，随后 Somnus 会自动重新打开。',
    errorTitle: '更新未完成',
    errorBody: '没有数据丢失。你可以现在重试。',
    blockerTitle: '关闭本地预览以更新 Somnus？',
    blockerBody: 'Somnus 需要在更新前停止这些本地预览。这不会修改或删除你的文件。',
    foreignBlockerTitle: '关闭其他进程以更新 Somnus',
    foreignBlockerBody: 'Somnus 无法安全地自动关闭这些进程。请关闭拥有这些进程的应用、终端或服务，然后重试更新。',
    mixedBlockerBody: 'Somnus 可以关闭下方列出的本地预览。其他进程必须手动关闭，更新才能继续。',
  ...
  guidedGreeting: {
    line: '来了，进来吧。我是 Somnus。给我两分钟，把这里按你的习惯收拾一下，然后我们找件你真正想做的事来做。\n\n先说，我该怎么称呼你？',
  ...
  install: {
    ...
    setupChoiceTitle: '设置 Somnus',
    setupChoiceDesc: '将此应用连接到你已运行的 Somnus 网关，或在这台电脑上本地安装 Somnus。',
    connectExistingTitle: '连接到现有 Somnus',
    connectExistingShort: '连接现有环境',
    connectExistingDesc: '使用会话令牌或浏览器登录连接远程后端。不会启动本地安装。',
    installLocalTitle: '本地安装 Somnus',
    installLocalDesc: '下载 Somnus，创建 Python 环境，并在这台电脑上运行后端。',
    localStartUnavailable: '无法启动本地安装。请重启 Somnus 后重试。',
    remoteSetupTitle: '连接到现有 Somnus',
    ...
    backToSetup: '返回',
    failedTitle: '安装失败',
    settingUpTitle: '正在设置 Somnus',
    finishingTitle: '正在收尾',
    failedDesc:
      '某个安装步骤失败。在 Windows 上，如果另一个 Somnus CLI 或桌面实例正在运行，可能会出现这种情况。请停止正在运行的 Somnus 实例后重试。可查看下面的详情或 desktop 日志中的完整记录。',
    activeDesc: '这是一次性设置。Somnus 安装器正在下载依赖并配置你的机器。之后启动会跳过此步骤。',
  ...
  onboarding: {
    ...
    somnusSignInDesc: '在浏览器中登录或创建免费账户。完成后 Somnus 会自动连接。',
    somnusSignInWaiting: '正在等待浏览器…',
    somnusUseKey: '改用密钥',
    somnusSignInFailed: '登录未完成，请重试。',
    somnusAccountNav: 'Somnus 账户',
    somnusAccountTitle: '你的 Somnus 账户',
    somnusAccountDesc: '所有模型都通过你的 Somnus 账户运行，并从 Somnus 余额扣费。可在 Somnus 网站充值或查看用量。',
    ...
    somnusAccountUsage30d: (spend, requests) => `过去 30 天已用 ${spend} · ${requests} 次请求`,
    somnusAccountTopUp: '充值',
    somnusAccountLowCredit: '余额已用完。充值后即可继续使用 Somnus。',
    somnusAccountSignOut: '退出登录',
    somnusAccountSignOutTitle: '退出 Somnus 登录？',
    somnusAccountSignOutDesc: '在你重新登录之前，这台电脑上的 Somnus 不会再使用你的账户。聊天和设置会保留，余额也会保留在你的账户中。',
    somnusAccountSignedOut: '已退出 Somnus 登录',
    somnusAccountNotSignedIn: '这台电脑上的 Somnus 尚未登录。',
    somnusAccountExpired: '登录已过期。请重新登录以继续使用 Somnus。',
    somnusAccountUnavailable: '暂时无法加载余额。',
    ...
    recommended: '推荐',
    connected: '已连接',
    featuredPitch: '一个订阅，300+ 前沿模型 — 运行 Somnus 的推荐方式',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: '自托管',
        description: '将 Somnus 指向本地或自托管的 OpenAI 兼容端点 (vLLM、llama.cpp、Ollama 等)。'
      }
    },
    backToSignIn: '返回登录',
    getKey: '获取密钥',
    ...
    flowSubtitles: {
      ...
      device_code: '在浏览器中打开验证页面 — Somnus 会自动连接',
    ...
    authorizeThere: '请在那里授权 Somnus。',
    copyAuthCode: '复制授权码并粘贴到下面。',
    pasteAuthCode: '粘贴授权码',
    reopenAuthPage: '重新打开授权页面',
    autoBrowser: provider => `已在浏览器中打开 ${provider}。请在那里授权 Somnus，连接会自动完成，无需复制或粘贴。`,
  ...
  freeTier: {
    ...
    alreadySignedInBody: '此 Somnus 已登录 Nous 账户。'
  },
  ...
  preview: {
    ...
    largeTitle: '此文件较大',
    largeBody: (label, size) => `${label} 大小为 ${size}。Somnus 只会显示前 512 KB。`,
    previewAnyway: '仍然预览',
    ...
    web: {
      ...
      failedToLoad: '预览加载失败',
      tryAgain: '重试',
      restarting: 'Somnus 正在重启...',
      askRestart: '让 Somnus 重启服务器',
      lookingRestart: taskId => `Somnus 正在查找要重启的预览服务器 (${taskId})`,
      restartingTitle: '正在重启预览服务器',
      restartingMessage: 'Somnus 正在后台工作。可在预览控制台查看进度。',
      ...
      blankPageBody: '在上方输入地址开始浏览，或让 Somnus 打开一个页面。',
      finishedRestarting: message => `Somnus 已完成预览服务器重启${message ? `: ${message}` : ''}`,
      failedRestarting: message => `服务器重启失败：${message}`,
      unknownError: '未知错误',
      restartedTitle: '预览服务器已重启',
      reloadingNow: '正在重新加载预览。',
      restartFailedTitle: '预览重启失败',
      restartFailedMessage: 'Somnus 无法重启服务器。',
      stillWorking: 'Somnus 仍在工作，但还没有收到重启结果。服务器命令可能正在前台运行。',
  ...
  interfaceMode: {
    title: '界面模式',
    hint: '只改变显示的内容，不改变 Somnus 的能力。',
    sessionNote: '由简洁模式设定。此处的更改仅在本次会话内生效；切换到高级模式即可保留为你的设置。',
    simple: {
      label: '简洁',
      description: '用于与 Somnus 对话。只有侧边栏和聊天；没有终端、文件或差异面板。'
    },
    advanced: {
      label: '高级',
  ...
  assistant: {
    ...
    approval: {
      gatewayDisconnected: 'Somnus 网关未连接',
      sendFailed: '无法发送审批响应',
      run: '运行',
      command: '命令',
      moreOptions: '更多审批选项',
  ...
  prompts: {
    ...
    vaultSaveDesc: origin =>
      `Somnus 到达了 ${origin} 的登录页，但没有为它保存的登录信息。在此输入一次；它将在本机加密保存并直接填入页面，模型永远看不到密码。`,
    ...
    vaultCodeDesc: site =>
      `${site} 要求输入一次性验证码（短信、邮件或验证器应用）。在此输入，Somnus 会将其填入页面；模型永远看不到它。`,
    vaultCodeLabel: '验证码',
    vaultCodeFootnote: '提示：在“设置 → 密码与登录”中为该登录保存验证器密钥后，Somnus 会自动填写验证码。',
    vaultCodeSkip: '跳过',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '重启 Somnus 桌面版后可使用保存图片。',
    restartToSaveImages: '重启 Somnus 桌面版以保存图片',
  ...
  tips: {
    ...
    items: {
      ...
      skills: {
        title: '教一次就够',
        text: '技能是一组说明文件，Somnus 会在需要时自行加载。'
      },
      messaging: {
        title: '离开电脑也能用',
      ...
      profiles: {
        title: '配置文件彼此独立',
        text: '每个都是独立的 Somnus — 自己的密钥、记忆和会话。'
      },
      'composer-mentions': {
        title: '附件与命令',
