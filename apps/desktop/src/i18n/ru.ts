...
export const ru = defineLocale({
  sessionImport: {
    ...
    subtitle: 'Перенесите разговор в Somnus и продолжите с того места, где остановились.',
    ...
    chooseHelp: 'Выберите сессию, чтобы прочитать историю перед импортом в Somnus.',
    ...
    you: 'Вы',
    snapshot: 'Этот разговор уже есть в Somnus. Откройте существующую копию, чтобы продолжить.',
    copyNotice:
      'Копируется текст разговора. Исходные файлы не меняются. Вывод инструментов и рассуждения не переносятся.',
    importing: 'Импорт…',
    open: 'Открыть в Somnus',
    continue: 'Продолжить в Somnus',
  ...
  boot: {
    ...
    steps: {
      ...
      loadingSettings: 'Загрузка настроек Somnus',
      loadingSessions: 'Загрузка последних сеансов',
      retryingRemoteBackend: 'Переподключение к удалённому бэкенду Somnus…',
      startingDesktopConnection: 'Запуск подключения приложения',
      startingHermesDesktop: 'Запуск Somnus…'
    },
    errors: {
      backgroundExited: 'Фоновый процесс Somnus завершён.',
      backgroundExitedDuringStartup: 'Фоновый процесс Somnus завершился при запуске.',
    ...
    failure: {
      title: 'Somnus не удалось запустить',
      description:
  ...
  notifications: {
    ...
    backendOutOfDateMessage:
      'Ваш бэкенд Somnus старше этой сборки приложения и может работать некорректно. Обновите их, чтобы они совпали.',
    installMethodUnsupportedTitle: 'Неподдерживаемый способ установки',
    updateHermes: 'Обновить Somnus',
    ...
    somnusUpdateReadyTitle: 'Обновление Somnus готово',
    somnusUpdateReadyMessage: v => `Somnus ${v} загружен. Перезапустите, чтобы завершить обновление — это займёт несколько секунд.`,
    somnusUpdateRestart: 'Перезапустить',
    mcp: {
    ...
    errors: {
      ...
      methodNotAllowed:
        'Бэкенд приложения отклонил запрос (405 Method Not Allowed). Попробуйте перезапустить Somnus.',
    ...
    native: {
      ...
      inputBody: 'Somnus ожидает ваш ответ.',
      turnDoneTitle: 'Somnus завершил',
      turnDoneBody: '',
  ...
  settings: {
    ...
    resetConfirm: 'Сбросить все настройки к значениям Somnus по умолчанию?',
    ...
    notifications: {
      ...
      focusedHint: 'Оповещения о завершении срабатывают только пока Somnus в фоне.',
      kinds: {
        approval: {
        ...
        input: {
          label: 'Нужен ввод',
          description: 'Somnus задал вопрос или требует пароль/секрет.'
        },
        turnDone: {
          label: 'Ответ готов',
          description: 'Ход завершился, пока Somnus был в фоне.'
        },
        turnError: {
          label: 'Ход не удался',
        ...
        plugin: {
          ...
          description: 'Плагин приложения отправил уведомление, пока Somnus был в фоне.'
        }
      },
    ...
    appearance: {
      ...
      colorModeDesc: 'Выберите фиксированный режим или позвольте Somnus следовать настройкам системы.',
      ...
      reactionsDesc: 'Эмодзи-тапбеки в стиле iMessage — реагируйте на сообщения, и Somnus сможет реагировать на ваши.',
      ...
      pet: {
        title: 'Питомец',
        intro:
          'Заберите анимированного питомца из petdex, который парит над приложением и реагирует на действия Somnus — «бегает», пока выполняются инструменты, радуется успеху и хмурится при ошибках.',
        restartHint:
          'Питомцам нужен быстрый перезапуск — текущее приложение запустилось до появления этой функции. Выйдите из Somnus и откройте снова, затем вернитесь сюда.',
        on: 'Вкл',
        off: 'Выкл',
        scaleTitle: 'Размер',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      agent: {
        ...
        maxTurns: 'Верхний предел ходов с вызовами инструментов, после которого Somnus останавливает запуск.'
      },
      terminal: {
      ...
      fileReadMaxChars: 'Максимальное число символов, которые Somnus может прочитать из одного запроса к файлу.',
      approvals: {
        mode: 'Как Somnus обрабатывает команды, требующие явного подтверждения.',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'Когда Somnus обновляет себя из приложения (без запроса в терминале), сохранять локальные правки исходников (stash) или выбрасывать (discard). Обновления из терминала всегда спрашивают.'
      }
    }),
    about: {
      heading: 'Somnus',
      ...
      bundleOutOfSyncDesc:
        'Рантайм Somnus обновлён, но само приложение — ещё старая сборка: новые функции интерфейса (например, Bot Mode) не появятся до обновления. Запустите обновление ниже, чтобы пересобрать приложение. Если предупреждение не исчезнет, переустановите с последнего установщика.',
      ...
      automaticUpdatesDesc: 'Somnus автоматически проверяет обновления в фоне и сообщает, когда они готовы.',
    ...
    config: {
      ...
      minimizeToTrayDesc:
        'Сворачивание окон или закрытие главного окна скрывает их в системном трее (строке меню macOS), оставляя Somnus работать. Для выхода выберите «Выйти из Somnus» в меню трея или нажмите Cmd+Q. По умолчанию выключено; действует только на этом устройстве.',
      ...
      loading: 'Загрузка конфигурации Somnus…',
    ...
    hudModifier: {
      ...
      permission:
        'Разрешите Somnus в Системных настройках → Конфиденциальность и безопасность → Мониторинг ввода, затем повторите попытку. Жест не записывает нажатия клавиш и не снимает экран.',
      unavailable:
        'Вспомогательная программа жеста HUD не запустилась или неожиданно остановилась. Повторите попытку или перезапустите Somnus. Обычное сочетание HUD по-прежнему работает внутри Somnus.',
      missingHelper:
        'В этой установке Somnus отсутствует вспомогательная программа жеста HUD. Обновите или переустановите Somnus и повторите попытку.',
      unsupportedSession:
    ...
    screenshot: {
      ...
      enabledDesc:
        'Нажмите обе клавиши Command одновременно в любом приложении, чтобы снять его переднее окно и прикрепить снимок к текущему черновику Somnus. Автоматической отправки нет. По умолчанию выключено; действует только на этом Mac. Окно может содержать конфиденциальные данные — проверьте вложение перед отправкой.',
      ...
      inputPermission:
        'Разрешение на мониторинг ввода позволяет Somnus распознавать обе клавиши Command, когда активно другое приложение. Разрешите Somnus доступ в Системных настройках → Конфиденциальность и безопасность → Мониторинг ввода, затем вернитесь сюда и повторите попытку.',
      screenPermission:
        'Разрешение на запись экрана позволяет Somnus снимать переднее окно приложения при использовании этого сочетания. Разрешите Somnus доступ в Системных настройках → Конфиденциальность и безопасность → Запись экрана, затем вернитесь сюда и повторите попытку. Перезапустите Somnus, если macOS попросит.',
    ...
    quickEntry: {
      ...
      enabledDesc:
        'Глобальным горячим ключом вызывайте маленький композер откуда угодно и отправляйте запрос, не открывая Somnus.',
    ...
    connections: {
      ...
      intro:
        'Управляйте этим устройством и всеми шлюзами Somnus, до которых можно дотянуться через удалённые, SSH или Cloud-соединения.',
      stagedNote:
      ...
      updateSkippedCloud: 'Управляется Somnus Cloud',
      kindLocal: 'Локальный',
      kindRemote: 'Удалённый шлюз',
      kindCloud: 'Somnus Cloud',
      kindSsh: 'SSH',
      kindLocalDesc: 'Рантайм Somnus, которым управляет это приложение.',
      kindRemoteDesc: 'Шлюз Somnus, доступный по HTTP(S) — LAN, Tailscale или интернет.',
      kindCloudDesc: 'Хостинговая инстанция, обнаруженная через ваш аккаунт Somnus Cloud.',
      kindSshDesc: 'Установка Somnus, доступная по SSH.',
      labelTitle: 'Имя',
      labelDesc:
      ...
      headersDesc:
        'Отправляются с каждым HTTP- и WebSocket-запросом к этому шлюзу — например, для прокси доступа вроде Cloudflare Access (CF-Access-Client-Id / CF-Access-Client-Secret). Значения хранятся зашифрованными. Заголовки, которыми управляет Somnus (Authorization, Cookie, Host…), игнорируются.',
      ...
      cloudAddHint:
        'Подсказка: вход в Somnus Cloud выше автоматически обнаруживает ваших агентов — эту форму используйте только для ручной регистрации известного URL инстанса.',
    ...
    gateway: {
      ...
      intro:
        'По умолчанию — локальный. Используйте удалённый, когда приложение должно управлять бэкендом Somnus в другом месте. Соединения шлюзов — на уровне машины; профили обнаруживаются из подключённых шлюзов.',
      ...
      localDesc: 'Запускает приватный бэкенд Somnus на localhost. Это значение по умолчанию, работает офлайн.',
      remoteTitle: 'Удалённый шлюз',
      remoteDesc: 'Подключает это приложение к удалённому бэкенду Somnus.',
      remoteAuthHint:
      ...
      cloudDesc: 'Войдите в Somnus Cloud один раз и выбирайте агентов из своего аккаунта — без вставки URL.',
      cloudSignInTitle: 'Somnus Cloud',
      cloudSignIn: 'Войти в Somnus Cloud',
      cloudSignedIn: 'Вы вошли в Somnus Cloud',
      cloudNeedsSignIn: 'Войдите в Somnus Cloud, чтобы обнаружить агентов в вашем аккаунте.',
      ...
      cloudDiscoverFailed: 'Не удалось загрузить агентов Somnus Cloud',
      cloudConnectFailed: 'Не удалось подключиться к этому агенту',
      cloudSignInFailed: 'Не удалось войти в Somnus Cloud',
      cloudSignedOutTitle: 'Выход из Somnus Cloud',
      cloudSignedOutMessage: 'Сессия Somnus Cloud сброшена.',
      ...
      restartingMessage: 'Somnus переподключится с сохранёнными настройками — оболочка останется открытой.',
      savedMessage: 'Сохранено для следующего перезапуска.',
      connectedTo: (baseUrl, version) => `Подключено к ${baseUrl}${version ? ` · Somnus ${version}` : ''}`,
      ...
      sshDesc:
        'Somnus запускается на удалённой машине по SSH и туннелируется в это приложение — ничего не нужно запускать или открывать самим. Требуется рабочая SSH-аутентификация по ключу на хост.',
      sshTrustHint:
      ...
      sshReachable: (host, platform) => `Доступен: ${host} (${platform}) — Somnus найден`,
      ...
      sshErrAuth:
        'SSH-аутентификация не удалась. Загрузите ключ в ssh-agent (ssh-add) или задайте IdentityFile в ~/.ssh/config — Somnus запускает ssh в неинтерактивном режиме.',
      sshErrHostKey:
      ...
      sshErrUpdateRequired: 'Перед подключением через SSH обновите Somnus на удалённой машине.',
    ...
    mcp: {
      ...
      deepLinkDescription:
        'Ссылка запросила добавить этот MCP-сервер в Somnus. Проверьте конфигурацию ниже — она пришла из ссылки, а не из Somnus.',
    ...
    providers: {
      ...
      intro:
        'Войдите по подписке — копировать API-ключ не нужно. Somnus проведёт вход в браузере прямо здесь, в приложении.',
      ...
      localEndpoint: {
        ...
        description: 'Направьте Somnus на любой OpenAI-совместимый эндпоинт (Zyphra, vLLM, llama.cpp, Ollama и т. д.).'
      },
  ...
  starmap: {
    ...
    emptyDesc: 'По мере того как Somnus создаёт навыки и память для вашей работы, они появятся здесь.',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: 'Перезапустите Somnus, чтобы использовать питомцев — бэкенд старше этой функции.',
    ...
    generatePet: {
      ...
      staleBackend: 'Обновите Somnus, чтобы генерировать питомцев.',
      backgroundHint: 'Можно закрыть — Somnus уведомит, когда будет готово.',
    ...
    nav: {
      ...
      settings: { title: 'Настройки', detail: 'Настройка Somnus desktop' },
    ...
    hermesActiveSessions: (version, count) => `Somnus ${version} · Активные сеансы: ${count}`,
  ...
  messaging: {
    ...
    telegramQr: {
      ...
      subtitle:
        'Оба способа подключают бота под вашим контролем и сохраняют его данные только в этой установке Somnus.',
      quickSetup: 'Быстрая настройка',
      recommended: 'Рекомендуется',
      quickHelp:
        'Отсканируйте QR-код и подтвердите в Telegram. Somnus создаст бота и определит ваш Telegram ID автоматически.',
  ...
  profiles: {
    ...
    defaultDescription: 'Используется при запуске Somnus и для новых чатов. Профили существующих сессий не меняются.',
    ...
    deleting: 'Удаление...',
    createDesc: 'Профили — это независимые среды Somnus: отдельные настройки, навыки и SOUL.md.',
    nameLabel: 'Имя',
  ...
  modelAssignment: {
    saveFailed: 'Somnus не сохранил это изменение модели.'
  },
  ...
  cron: {
    ...
    newCron: 'Новый cron',
    emptyDescNew:
      'Запланируйте промпт, который будет выполняться по cron-выражению. Somnus выполнит его и доставит результаты в выбранное вами место.',
  ...
  sidebar: {
    ...
    projects: {
      ...
      staleBackend:
        'Обновите бэкенд Somnus, чтобы создавать проекты — ваш бэкенд старше этого desktop-приложения (Настройки → Обновления → Бэкенд).',
      deleteConfirm: 'Это удалит сохранённый проект из Somnus. Файлы, git-репозитории и worktrees не пострадают.',
      ...
      worktreeStaleBackend:
        'Обновите бэкенд Somnus, чтобы создавать worktrees по этому удалённому соединению — он старше git worktree API.',
  ...
  composer: {
    ...
    placeholderStarting: 'Запуск Somnus...',
    placeholderReconnecting: 'Переподключение к Somnus…',
    ...
    attachUrlDesc: 'Somnus загрузит страницу и добавит её как контекст для этого хода.',
  ...
  statusStack: {
    ...
    coding: {
      ...
      agentShip: 'Попросить Somnus открыть PR',
  ...
  updates: {
    stages: {
      ...
      pull: 'Почти готово…',
      pydeps: 'Завершаем…',
      update: 'Обновляем Somnus…',
      rebuild: 'Пересобираем desktop-приложение…',
      restart: 'Перезапускаем Somnus…',
    ...
    somnusAutoTitle: 'Somnus обновляется автоматически',
    somnusAutoMessage: 'Somnus сам следит за обновлениями. Новые версии загружаются в фоне и устанавливаются при следующем перезапуске Somnus.',
    unsupportedMessage: 'Эта версия Somnus не может обновлять себя из приложения.',
    connectionRetry: 'Проверьте соединение и попробуйте снова.',
    gitUnusable: 'Somnus не удалось запустить Git на этом компьютере, поэтому проверить обновления не получилось.',
    ...
    availableBody: 'Новая версия Somnus готова к установке.',
    availableTitleBackend: 'Доступно обновление бэкенда',
    availableBodyBackend: 'Новая версия подключённого бэкенда Somnus готова к установке.',
    ...
    manualBody:
      'Somnus установлен из командной строки, поэтому обновления тоже выполняются там. Вставьте это в терминал:',
    manualPickedUp: 'Somnus подхватит новую версию при следующем запуске.',
    guiSkewTitle: 'Обновите desktop-приложение',
    guiSkewBody:
      'Бэкенд обновлён, но пакет этого desktop-приложения не изменился. Обновите или переустановите desktop-приложение Somnus (ваш AppImage / .deb / .rpm), чтобы версии совпали.',
    copy: 'Копировать',
    copied: 'Скопировано',
    done: 'Готово',
    applyingBody:
      'Обновлятор Somnus возьмёт процесс на себя в своём окне и автоматически перезапустит Somnus, когда закончит. Пожалуйста, не открывайте Somnus вручную во время обновления.',
    applyingBodyBackend:
      'Удалённый бэкенд применяет обновление и перезапустится. Somnus переподключится автоматически, когда он снова заработает.',
    applyingClose: 'Это окно закроется во время обновления, затем Somnus откроется сам.',
    errorTitle: 'Обновление не завершилось',
    errorBody: 'Не переживайте — ничего не потеряно. Можно попробовать снова прямо сейчас.',
    blockerTitle: 'Закрыть локальные предпросмотры, чтобы обновить Somnus?',
    blockerBody:
      'Somnus нужно остановить эти локальные предпросмотры перед обновлением. Ваши файлы не будут изменены или удалены.',
    foreignBlockerTitle: 'Закрыть другие процессы, чтобы обновить Somnus',
    foreignBlockerBody:
      'Somnus не может безопасно закрыть эти процессы автоматически. Закройте приложение, терминал или службу, которой принадлежит каждый из них, и повторите обновление.',
    mixedBlockerBody:
      'Somnus может закрыть локальные предпросмотры, перечисленные ниже. Остальные процессы нужно закрыть вручную, прежде чем обновление сможет продолжиться.',
  ...
  guidedGreeting: {
    line: 'Заходите. Я Somnus. Дайте мне пару минут — обустрою тут всё под вас, а потом займёмся тем, что вам правда нужно.\n\nДля начала: как к вам обращаться?',
  ...
  install: {
    ...
    oneTimeTitle: 'Somnus требует одноразовой установки',
    ...
    setupChoiceTitle: 'Настройка Somnus',
    setupChoiceDesc:
      'Подключите это приложение к уже работающему шлюзу Somnus или установите Somnus локально на этот компьютер.',
    connectExistingTitle: 'Подключиться к существующему Somnus',
    ...
    installLocalTitle: 'Установить Somnus локально',
    installLocalDesc: 'Скачайте Somnus, создайте его Python-окружение и запустите бэкенд на этом компьютере.',
    localStartUnavailable: 'Не удалось начать локальную установку. Перезапустите Somnus и попробуйте снова.',
    remoteSetupTitle: 'Подключиться к существующему Somnus',
    ...
    settingUpTitle: 'Настройка Somnus',
    finishingTitle: 'Завершаем',
    failedDesc:
      'Один из шагов установки завершился ошибкой. На Windows это может произойти, если запущена другая инстанция Somnus CLI или desktop. Остановите все работающие инстанции Somnus и повторите. Подробности — ниже или в журнале desktop.',
    activeDesc:
      'Это одноразовая настройка. Установщик Somnus скачивает зависимости и настраивает вашу машину. При следующих запусках этот шаг будет пропущен.',
  ...
  onboarding: {
    headerTitle: 'Настроим для вас Somnus',
    headerDesc: 'Войдите в аккаунт Somnus, чтобы начать общение.',
    somnusKeyTitle: 'Вставьте ключ Somnus',
    somnusKeyDesc: 'Ключ подключает Somnus ко всем моделям. Его можно найти в аккаунте Somnus.',
    somnusKeyInvalid: 'Этот ключ не подошёл. Проверьте его и попробуйте снова.',
    somnusSignIn: 'Войти через Somnus',
    somnusSignInDesc: 'Войдите или создайте бесплатный аккаунт в браузере. Когда закончите, Somnus подключится сам.',
    somnusSignInWaiting: 'Ожидание браузера…',
    somnusUseKey: 'Использовать ключ',
    somnusSignInFailed: 'Вход не завершён. Попробуйте ещё раз.',
    somnusAccountNav: 'Аккаунт Somnus',
    somnusAccountTitle: 'Ваш аккаунт Somnus',
    somnusAccountDesc: 'Все модели работают через ваш аккаунт Somnus и оплачиваются с баланса Somnus. Пополнить баланс и посмотреть расходы можно на сайте Somnus.',
    ...
    somnusAccountUsage30d: (spend, requests) => `${spend} за последние 30 дней · запросов: ${requests}`,
    somnusAccountTopUp: 'Пополнить',
    somnusAccountLowCredit: 'Кредит закончился. Пополните баланс, чтобы продолжить пользоваться Somnus.',
    somnusAccountSignOut: 'Выйти',
    somnusAccountSignOutTitle: 'Выйти из Somnus?',
    somnusAccountSignOutDesc: 'Somnus на этом компьютере перестанет использовать ваш аккаунт, пока вы снова не войдёте. Чаты и настройки останутся здесь, а кредит — на вашем аккаунте.',
    somnusAccountSignedOut: 'Вы вышли из Somnus',
    somnusAccountNotSignedIn: 'Somnus на этом компьютере не выполнил вход.',
    somnusAccountExpired: 'Срок входа истёк. Войдите снова, чтобы продолжить.',
    somnusAccountUnavailable: 'Сейчас не удаётся загрузить баланс.',
    preparingInstall: 'Somnus завершает установку. Обычно это занимает меньше минуты при первом запуске.',
    starting: 'Запускаем Somnus…',
    ...
    recommended: 'Рекомендуется',
    connected: 'Подключено',
    featuredPitch: 'Одна подписка, 300+ передовых моделей — рекомендуемый способ запускать Somnus',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: 'self-hosted',
        description:
          'Укажите Somnus локальный или self-hosted OpenAI-совместимый endpoint (vLLM, llama.cpp, Ollama и т.д.).'
      }
    },
    ...
    flowSubtitles: {
      ...
      device_code: 'Откроет страницу подтверждения в браузере — Somnus подключится автоматически',
    ...
    authorizeThere: 'Авторизуйте Somnus там.',
    ...
    autoBrowser: provider =>
      `Мы открыли ${provider} в вашем браузере. Авторизуйте Somnus там, и подключение произойдёт автоматически — ничего копировать и вставлять не нужно.`,
  ...
  preview: {
    ...
    largeBody: (label, size) => `${label} — ${size}. Somnus покажет только первые 512 КБ.`,
    ...
    web: {
      ...
      tryAgain: 'Попробовать снова',
      restarting: 'Somnus перезапускается...',
      askRestart: 'Попросить Somnus перезапустить сервер',
      lookingRestart: taskId => `Somnus ищет сервер предпросмотра для перезапуска (${taskId})`,
      restartingTitle: 'Перезапуск сервера предпросмотра',
      restartingMessage: 'Somnus работает в фоне. Следите за прогрессом в консоли предпросмотра.',
      ...
      blankPageBody: 'Введите адрес выше, чтобы просматривать, или попросите Somnus открыть страницу.',
      finishedRestarting: message => `Somnus завершил перезапуск сервера предпросмотра${message ? `: ${message}` : ''}`,
      ...
      restartFailedMessage: 'Somnus не смог перезапустить сервер.',
      stillWorking:
        'Somnus всё ещё работает, но результата перезапуска пока нет. Команда сервера может выполняться в foreground.',
  ...
  interfaceMode: {
    ...
    hint: 'Меняет то, что показано, а не то, что умеет Somnus.',
    sessionNote:
      'Задано простым режимом. Изменение здесь действует до конца сеанса; переключитесь в расширенный, чтобы сделать его своим.',
    simple: {
      label: 'Простой',
      description: 'Для общения с Somnus. Боковая панель и чат; без терминала, файлов и панелей diff.'
    },
    advanced: {
      label: 'Расширенный',
  ...
  assistant: {
    thread: {
      ...
      loadingResponse: 'Somnus загружает ответ',
    ...
    approval: {
      gatewayDisconnected: 'Шлюз Somnus не подключён',
  ...
  prompts: {
    ...
    secretDesc: 'Somnus нужны учётные данные, чтобы продолжить.',
  ...
  desktop: {
    ...
    restartToUseSaveImage: 'Перезапустите Somnus, чтобы использовать «Сохранить изображение».',
    restartToSaveImages: 'Перезапустите Somnus, чтобы сохранять изображения',
