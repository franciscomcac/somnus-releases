...
export const zhHant = defineLocale({
  sessionImport: {
    title: '從其他應用程式繼續',
    subtitle: '將對話匯入 Somnus，接著上次的進度繼續。',
    action: '匯入工作階段',
    readingFrom: '讀取自',
  ...
  boot: {
    ...
    steps: {
      ...
      loadingSettings: '正在載入 Somnus 設定',
      loadingSessions: '正在載入最近工作階段',
      retryingRemoteBackend: '正在重新連線遠端 Somnus 後端…',
      startingDesktopConnection: '正在啟動桌面連線',
      startingHermesDesktop: '正在啟動 Somnus…'
    },
    errors: {
      backgroundExited: 'Somnus 背景程序已結束。',
      backgroundExitedDuringStartup: 'Somnus 背景程序在啟動期間結束。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage: '您的 Somnus 後端早於目前的桌面版本，可能無法正常運作。請更新以保持一致。',
    ...
    seeWhatsNew: '查看新增內容',
    somnusUpdateReadyTitle: 'Somnus 更新已就緒',
    somnusUpdateReadyMessage: v => `Somnus ${v} 已下載。重新啟動即可完成更新，只需幾秒鐘。`,
    somnusUpdateRestart: '立即重新啟動',
    mcp: {
    ...
    errors: {
      ...
      methodNotAllowed: '桌面後端拒絕了該請求 (405 Method Not Allowed)。請嘗試重新啟動 Somnus。',
  ...
  settings: {
    ...
    resetConfirm: '要將所有設定恢復為 Somnus 預設值嗎？',
    exportFailed: '匯出失敗',
    resetFailed: '重設失敗',
    nav: {
      providers: '提供方',
    ...
    vault: {
      ...
      otpHint: '啟用兩步驟驗證時網站顯示的「設定金鑰」。儲存後 Somnus 會自動產生驗證碼。',
      ...
      sources: {
        ...
        notInstalled: name => `未偵測到。安裝 ${name} 命令列工具並登入後，Somnus 會自動偵測。`,
        disabledDesc: '已偵測到，但已為 Somnus 關閉。',
        lockedDesc: '已偵測到。代理需要登入資訊時會請你解鎖，也可立即解鎖。',
        unlockedDesc: '本工作階段已解鎖。閒置 30 分鐘或關閉 Somnus 後會自動鎖定。',
        statusLocked: '已鎖定',
    ...
    notifications: {
      ...
      focusedHint: '完成提醒僅在 Somnus 位於背景時觸發。',
      kinds: {
        approval: {
          label: '需要核准',
        ...
        input: {
          label: '需要輸入',
          description: 'Somnus 提出了問題，或需要密碼或密鑰。'
        },
        turnDone: {
          label: '回覆就緒',
          description: 'Somnus 在背景時完成了一輪對話。'
        },
        turnError: {
          label: '本輪失敗',
          description: '背景回合錯誤。'
        ...
        plugin: {
          label: '外掛通知',
          description: 'Somnus 在背景時，桌面外掛傳送了通知。'
        }
      },
      test: '傳送測試通知',
      testTitle: 'Somnus',
    ...
    appearance: {
      ...
      colorMode: '色彩模式',
      colorModeDesc: '選擇固定模式，或讓 Somnus 跟隨系統設定。',
      ...
      reactionsTitle: '訊息回應',
      reactionsDesc: 'iMessage 風格的表情回應 — 你可以對訊息做出回應，Somnus 也能回應你的訊息。',
      tipsTitle: '應用程式內提示',
      tipsDesc: '偶爾顯示來自應用程式和 Somnus 的提示，每則提示只出現一次。開始使用滿30天後自動關閉，你可以重新開啟。',
      tipsReset: (count: number) => `再次顯示 ${count} 則提示`,
      toursTitle: '導覽',
      toursDesc: '讓 Somnus 逐步標示每個位置，帶你認識應用程式。開始使用滿30天後自動關閉，你可以重新開啟。',
      ...
      pet: {
        title: '寵物',
        intro:
          '領養一隻懸浮在應用上的 petdex 動畫寵物，它會根據 Somnus 的狀態做出反應——工具執行時奔跑、成功時歡呼、出錯時沮喪。',
        restartHint: '寵物功能需要重新啟動——目前執行的應用在此功能加入前啟動。請結束並重新開啟 Somnus，然後回到此處。',
        scaleTitle: '大小',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone: 'Somnus 需要本機時間上下文時使用。留空則使用系統時區。',
      agent: {
        imageInputMode: '控制圖片附件如何傳送給模型。',
        maxTurns: 'Somnus 停止一次執行前的工具呼叫輪次上限。'
      },
      terminal: {
      ...
      fileReadMaxChars: 'Somnus 單次檔案讀取可讀取的最大字元數。',
      approvals: {
        mode: 'Somnus 如何處理需要明確批准的指令。',
      ...
      browser: {
        useRealProfile:
          '本機瀏覽會使用你的真實登入狀態。Somnus 會將預設瀏覽器的設定（Cookie、登入資訊與偏好）複製成受管理的快照，再以內建的 Chromium 驅動它——不會直接開啟你正在使用的設定檔，且每次執行都會從目前的設定檔重新整理副本。設定雲端瀏覽器後端時，也允許代理視需要開啟本機真實設定檔工作階段。僅支援 Chromium 系瀏覽器（Chrome、Edge、Brave、Brave Origin、Chromium）；若預設瀏覽器並非 Chromium 系，會顯示明確錯誤。預設關閉。'
      },
      voice: {
        autoTts: '自動朗讀助手回覆。',
        voiceChatMode:
          'chained：語音轉文字 → Somnus → 文字轉語音，使用下方的提供方。gpt-live：由全雙工 OpenAI 語音模型（gpt-live-1）負責聆聽與說話，並將每個實際請求交給 Somnus——由你選擇的任意模型使用完整工具集作答。需要 OpenAI API 金鑰；語音層每分鐘收費 $0.05。',
        gptLive: {
          voice: 'GPT-Live 模式使用的音色，可填入自訂音色 ID。',
          instructions: '附加至即時語音人設的句子（語氣、語速、語言）。Somnus 會保留自己的系統提示詞。'
        }
      },
      stt: {
      ...
      updates: {
        nonInteractiveLocalChanges:
          'Somnus 從應用程式內更新自身時，保留本機原始碼變更（stash）或丟棄（discard）。終端機更新一律會詢問。'
      }
    }),
    uninstallSection: {
      dangerZone: '危險操作',
      confirmUninstall: '確認解除安裝',
      uninstallHermes: '解除安裝 Somnus'
    },
    poolLimits: {
    ...
    about: {
      ...
      bundleOutOfSyncDesc:
        'Somnus 執行環境已更新,但桌面應用程式本身仍是舊建置——在應用程式更新之前,新的介面功能(如 Bot Mode)不會顯示。請執行下方的更新以重新建置應用程式。如果此警告仍未消除,請從最新的桌面安裝程式重新安裝。',
      bundleOutOfSyncAction: '取得安裝程式',
      bundleSwapPending: '重新啟動以完成更新',
      bundleSwapPendingDesc:
        '更新後的應用程式已安裝完成，只需重新啟動 Somnus 即可載入新版本。聊天記錄和設定不會受到影響。',
      bundleSwapPendingAction: '重新啟動 Somnus',
      updates: '更新',
      checkNow: '立即檢查',
      checking: '檢查中…',
      seeWhatsNew: '查看新增內容',
      ...
      automaticUpdatesDesc: 'Somnus 會在背景自動檢查更新，並在有可用更新時通知你。',
    ...
    config: {
      ...
      minimizeToTrayDesc:
        '最小化視窗或關閉主視窗時，將其隱藏至系統匣（macOS 上為選單列），讓 Somnus 繼續執行。透過系統匣選單中的「結束 Somnus」或 Cmd+Q 結束。預設關閉，僅適用於此裝置。',
    ...
    hudModifier: {
      ...
      permission: '請在系統設定 → 隱私權與安全性 → 輸入監控中允許 Somnus，然後重試。此手勢不會記錄按鍵或擷取畫面。',
      unavailable: 'HUD 手勢輔助程式無法啟動或意外停止。請重試或重新啟動 Somnus。Somnus 內原有的 HUD 快速鍵仍可使用。',
      missingHelper: '此 Somnus 安裝缺少 HUD 手勢輔助程式。請更新或重新安裝 Somnus，然後重試。',
      unsupportedSession: '此桌面工作階段不支援全域修飾鍵輕按事件。Linux 需要 X11；不支援 Wayland。'
    },
    screenshot: {
      enabledTitle: '螢幕截圖快速鍵',
      enabledDesc:
        '在任何應用程式中同時按下左右兩個 Command 鍵，即可擷取最前方的視窗並附加到目前的 Somnus 草稿。絕不會自動傳送。預設關閉，僅適用於這台 Mac。視窗可能包含敏感內容，請在傳送前檢查附件。',
      ...
      inputPermission:
        '輸入監控權限可讓 Somnus 在其他應用程式使用中時偵測兩個 Command 鍵。請在系統設定 → 隱私權與安全性 → 輸入監控中允許 Somnus，然後返回此處重試。',
      screenPermission:
        '螢幕錄製權限可讓 Somnus 在你使用此快速鍵時擷取最前方的應用程式視窗。請在系統設定 → 隱私權與安全性 → 螢幕錄製中允許 Somnus，然後返回此處重試。如果 macOS 提示，請重新啟動 Somnus。',
      openSettings: '開啟系統設定',
      retry: '重試',
    ...
    quickEntry: {
      enabledTitle: '快速輸入',
      enabledDesc: '用全域快速鍵在任何地方喚出一個小輸入框，無需開啟 Somnus 即可送出提示。',
    ...
    gateway: {
      ...
      envOverride: '環境變數覆寫',
      intro:
        'Somnus 預設會啟動自己的本機閘道。如果您希望此應用程式控制另一台機器或可信代理後面已執行的 Somnus 後端，請使用遠端閘道。閘道連線屬於本機層級設定；設定檔是從已連線的閘道中探索出來的。',
      ...
      localTitle: '本機閘道',
      localDesc: '在 localhost 啟動私有 Somnus 後端。這是預設方式，可離線使用。',
      remoteTitle: '遠端閘道',
      remoteDesc:
      ...
      savedTitle: '閘道設定已儲存',
      restartingMessage: 'Somnus 將使用已儲存的設定重新連線。',
      savedMessage: '已儲存，下次重新啟動後生效。',
      connectedTo: (baseUrl, version) => `已連線至 ${baseUrl}${version ? ` · Somnus ${version}` : ''}`,
      ...
      sshTitle: '透過 SSH 連線',
      sshDesc:
        'Somnus 會透過 SSH 在遠端啟動並以通道連線到本應用程式——無需自行啟動或公開任何服務。前提：已具備到該主機的金鑰 SSH 存取。',
      ...
      sshReachable: (host, platform) => `可連線：${host}（${platform}）——已找到 Somnus`,
    ...
    mcp: {
      ...
      deepLinkDescription: '一個連結要求將此 MCP 伺服器加入 Somnus。請檢查下方的完整設定——它來自該連結，而非 Somnus。',
    ...
    localModels: {
      ...
      runtimeInstalledDetail: (tag, backend) => `組建 ${tag}，${backend} 後端。Somnus 會為您啟動並管理伺服器。`,
    ...
    providers: {
      ...
      intro: '使用訂閱登入，無需複製 API 金鑰。Somnus 會在應用程式中為您完成瀏覽器登入。',
      connected: '已連線',
      collapse: '收合',
      ...
      localEndpoint: {
        title: '本地 / 自訂端點',
        description: '將 Somnus 指向任意 OpenAI 相容端點（Zyphra、vLLM、llama.cpp、Ollama 等）。'
      },
      loading: '正在載入提供方...'
    },
    sessions: {
    ...
    toolsets: {
      ...
      browserRealProfile: {
        ...
        prompt: {
          title: '讓網站保持登入狀態',
          body: '讓 Somnus 使用預設瀏覽器設定檔的快照進行瀏覽，網站開啟時即已登入。',
  ...
  starmap: {
    ...
    loading: '載入中…',
    emptyTitle: '尚無學習內容',
    emptyDesc: '當 Somnus 為你的工作建立技能與記憶時，會顯示在這裡。'
  },
  agents: {
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: '請重新啟動 Somnus 以使用寵物功能。',
      empty: '沒有符合的寵物。',
      turnOff: '關閉',
      turnOn: '開啟',
      installed: '已安裝',
    ...
    generatePet: {
      ...
      staleBackend: '請更新 Somnus 以生成寵物。',
      backgroundHint: '你可以關閉此視窗——完成後 Somnus 會通知你。',
    ...
    nav: {
      ...
      settings: { title: '設定', detail: '設定 Somnus 桌面端' },
    ...
    hermesActiveSessions: (version, count) => `Somnus ${version} · 活躍工作階段 ${count}`,
  ...
  messaging: {
    ...
    telegramQr: {
      ...
      subtitle: '兩種方式都會連接由你控制的機器人，憑證僅儲存在此 Somnus 安裝中。',
      quickSetup: '快速設定',
      recommended: '推薦',
      quickHelp: '掃描 QR 碼並在 Telegram 中確認。Somnus 會自動建立機器人並偵測你的 Telegram 使用者 ID。',
  ...
  profiles: {
    ...
    defaultDescription: '用於 Somnus 啟動和新建聊天。現有工作階段仍保留在各自的設定檔中。',
    ...
    deleting: '刪除中…',
    createDesc: '設定檔是獨立的 Somnus 環境：各自擁有獨立的設定、技能和 SOUL.md。',
    nameLabel: '名稱',
    cloneFrom: '複製來源',
    cloneFromNone: '無（空白）',
  ...
  cron: {
    ...
    newCron: '新排程工作',
    emptyDescNew: '按 cron 表達式排程一個提示詞。Somnus 會執行它，並將結果傳送至您選擇的目的地。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      createFailed: '無法建立專案',
      staleBackend: '請更新 Somnus 後端以建立專案——目前後端比桌面應用舊（設定 → 更新 → 後端）。',
      deleteConfirm: '這會從 Somnus 中移除已儲存的專案。檔案、git 儲存庫和工作樹維持不變。',
      startWork: '新增工作樹',
      ...
      worktreeStaleBackend: '請更新 Somnus 後端以在此遠端連線上建立工作樹 —— 該後端早於 git 工作樹 API。',
  ...
  composer: {
    ...
    placeholderStarting: '正在啟動 Somnus...',
    placeholderReconnecting: '正在重新連線至 Somnus…',
    ...
    attachUrlDesc: 'Somnus 將擷取該頁面並作為此回合的脈絡。',
  ...
  updates: {
    ...
    somnusAutoTitle: 'Somnus 會自動更新',
    somnusAutoMessage: 'Somnus 會自動保持最新。新版本會在背景下載，並在下次重新啟動 Somnus 時安裝。',
    unsupportedMessage: '此版本的 Somnus 無法在應用程式內自行更新。',
    connectionRetry: '請檢查網路連線後重試。',
    gitUnusable: 'Somnus 無法在這台電腦上執行 Git，因此無法檢查更新。',
    latestBody: '您正在執行最新版本。',
    latestBodyBackend: '後端正在執行最新版本。',
    allSetTitle: '已是最新版本',
    availableTitle: '有可用更新',
    availableBody: '新版 Somnus 已可安裝。',
    availableTitleBackend: '後端有可用更新',
    availableBodyBackend: '已連接的 Somnus 後端有新版本可安裝。',
    availableBodyNoChangelog: '已有新版本可用。此安裝方式無法顯示更新日誌。',
    updateNow: '立即更新',
    maybeLater: '稍後再說',
    moreChanges: count => `另有 ${count} 項變更。`,
    manualTitle: '從終端機更新',
    manualBody: '您是從命令列安裝的 Somnus，因此更新也需要在那裡執行。請將此指令貼到終端機：',
    manualPickedUp: '下次啟動 Somnus 時會使用新版本。',
    guiSkewTitle: '請更新桌面應用程式',
    guiSkewBody:
      '後端已更新，但此桌面應用程式套件未變更。請更新或重新安裝 Somnus 桌面應用程式（你的 AppImage / .deb / .rpm）以保持一致。',
    copy: '複製',
    copied: '已複製',
    done: '完成',
    applyingBody:
      'Somnus 更新程式會在自己的視窗中接管，並在完成後自動重新開啟 Somnus。更新期間請勿自行重新開啟 Somnus。',
    applyingBodyBackend: '遠端後端正在套用更新並將重新啟動。恢復後 Somnus 會自動重新連線。',
    applyingClose: '此視窗會在更新期間關閉，隨後 Somnus 會自動重新開啟。',
    errorTitle: '更新未完成',
    errorBody: '沒有資料遺失。您可以現在重試。',
    blockerTitle: '關閉本機預覽以更新 Somnus？',
    blockerBody: 'Somnus 需要在更新前停止這些本機預覽。這不會修改或刪除您的檔案。',
    foreignBlockerTitle: '關閉其他處理程序以更新 Somnus',
    foreignBlockerBody:
      'Somnus 無法安全地自動關閉這些處理程序。請關閉擁有這些處理程序的應用程式、終端機或服務，然後重試更新。',
    mixedBlockerBody: 'Somnus 可以關閉下方列出的本機預覽。其他處理程序必須手動關閉，更新才能繼續。',
  ...
  guidedGreeting: {
    line: '來了，進來吧。我是 Somnus。給我兩分鐘，把這裡按你的習慣整理一下，然後我們找件你真正想做的事來做。\n\n先說，我該怎麼稱呼你？',
  ...
  install: {
    ...
    setupChoiceTitle: '設定 Somnus',
    setupChoiceDesc: '將此應用程式連線到您已執行的 Somnus 閘道，或在這台電腦上本機安裝 Somnus。',
    connectExistingTitle: '連線到現有 Somnus',
    connectExistingShort: '連線現有環境',
    connectExistingDesc: '使用工作階段權杖或瀏覽器登入連線遠端後端。不會啟動本機安裝。',
    installLocalTitle: '本機安裝 Somnus',
    installLocalDesc: '下載 Somnus、建立 Python 環境，並在這台電腦上執行後端。',
    localStartUnavailable: '無法啟動本機安裝。請重新啟動 Somnus 後再試一次。',
    remoteSetupTitle: '連線到現有 Somnus',
    ...
    backToSetup: '返回',
    failedTitle: '安裝失敗',
    settingUpTitle: '正在設定 Somnus',
    finishingTitle: '正在收尾',
    failedDesc:
      '某個安裝步驟失敗。在 Windows 上，如果另一個 Somnus CLI 或桌面執行個體正在執行，可能會出現這種情況。請停止正在執行的 Somnus 執行個體後重試。可查看下方的詳細資訊或 desktop 記錄中的完整記錄。',
    activeDesc: '這是一次性設定。Somnus 安裝程式正在下載相依套件並設定您的電腦。之後啟動會略過此步驟。',
  ...
  onboarding: {
    ...
    somnusSignInDesc: '在瀏覽器中登入或建立免費帳戶。完成後 Somnus 會自動連線。',
    somnusSignInWaiting: '正在等待瀏覽器…',
    somnusUseKey: '改用金鑰',
    somnusSignInFailed: '登入未完成，請再試一次。',
    somnusAccountNav: 'Somnus 帳戶',
    somnusAccountTitle: '你的 Somnus 帳戶',
    somnusAccountDesc: '所有模型都透過你的 Somnus 帳戶執行，並從 Somnus 餘額扣款。可在 Somnus 網站儲值或查看用量。',
    ...
    somnusAccountUsage30d: (spend, requests) => `過去 30 天已用 ${spend} · ${requests} 次請求`,
    somnusAccountTopUp: '儲值',
    somnusAccountLowCredit: '餘額已用完。儲值後即可繼續使用 Somnus。',
    somnusAccountSignOut: '登出',
    somnusAccountSignOutTitle: '要登出 Somnus 嗎？',
    somnusAccountSignOutDesc: '在你重新登入之前，這台電腦上的 Somnus 不會再使用你的帳戶。聊天與設定會保留，餘額也會保留在你的帳戶中。',
    somnusAccountSignedOut: '已登出 Somnus',
    somnusAccountNotSignedIn: '這台電腦上的 Somnus 尚未登入。',
    somnusAccountExpired: '登入已過期。請重新登入以繼續使用 Somnus。',
    somnusAccountUnavailable: '暫時無法載入餘額。',
    ...
    recommended: '建議',
    connected: '已連線',
    featuredPitch: '一個訂閱，300+ 前沿模型 — 執行 Somnus 的建議方式',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: '自託管',
        description: '將 Somnus 指向本機或自託管的 OpenAI 相容端點（vLLM、llama.cpp、Ollama 等）。'
      }
    },
    backToSignIn: '返回登入',
    getKey: '取得金鑰',
    ...
    flowSubtitles: {
      ...
      device_code: '在瀏覽器中開啟驗證頁面 — Somnus 會自動連線',
    ...
    authorizeThere: '請在那裡授權 Somnus。',
    copyAuthCode: '複製授權碼並貼到下方。',
    pasteAuthCode: '貼上授權碼',
    reopenAuthPage: '重新開啟授權頁面',
    autoBrowser: provider => `已在瀏覽器中開啟 ${provider}。請在那裡授權 Somnus，連線會自動完成，無需複製或貼上。`,
  ...
  preview: {
    ...
    largeTitle: '此檔案較大',
    largeBody: (label, size) => `${label} 大小為 ${size}。Somnus 只會顯示前 512 KB。`,
    previewAnyway: '仍然預覽',
    ...
    web: {
      ...
      failedToLoad: '預覽載入失敗',
      tryAgain: '重試',
      restarting: 'Somnus 正在重新啟動...',
      askRestart: '請 Somnus 重新啟動伺服器',
      lookingRestart: taskId => `Somnus 正在尋找要重新啟動的預覽伺服器 (${taskId})`,
      restartingTitle: '正在重新啟動預覽伺服器',
      restartingMessage: 'Somnus 正在背景執行。可在預覽主控台查看進度。',
      ...
      blankPageBody: '在上方輸入網址開始瀏覽，或請 Somnus 開啟頁面。',
      finishedRestarting: message => `Somnus 已完成預覽伺服器重新啟動${message ? `：${message}` : ''}`,
      failedRestarting: message => `伺服器重新啟動失敗：${message}`,
      unknownError: '未知錯誤',
      restartedTitle: '預覽伺服器已重新啟動',
      reloadingNow: '正在重新載入預覽。',
      restartFailedTitle: '預覽重新啟動失敗',
      restartFailedMessage: 'Somnus 無法重新啟動伺服器。',
      stillWorking: 'Somnus 仍在執行，但尚未收到重新啟動結果。伺服器指令可能正在前台執行。',
  ...
  interfaceMode: {
    title: '介面模式',
    hint: '只改變顯示的內容，不改變 Somnus 的能力。',
    sessionNote: '由簡潔模式設定。此處的變更僅在本次工作階段內生效；切換到進階模式即可保留為你的設定。',
    simple: {
      label: '簡潔',
      description: '用於與 Somnus 對話。只有側邊欄和聊天；沒有終端機、檔案或差異面板。'
    },
    advanced: {
      label: '進階',
  ...
  assistant: {
    ...
    approval: {
      gatewayDisconnected: 'Somnus 閘道未連線',
      sendFailed: '無法傳送核准回應',
      run: '執行',
      command: '指令',
      moreOptions: '更多核准選項',
  ...
  prompts: {
    ...
    vaultSaveDesc: origin =>
      `Somnus 到達了 ${origin} 的登入頁，但沒有為它儲存的登入資訊。在此輸入一次；它會在本機加密儲存並直接填入頁面，模型永遠看不到密碼。`,
    ...
    vaultCodeDesc: site =>
      `${site} 要求輸入一次性驗證碼（簡訊、電子郵件或驗證器應用程式）。在此輸入，Somnus 會將其填入頁面；模型永遠看不到它。`,
    vaultCodeLabel: '驗證碼',
    vaultCodeFootnote: '提示：在「設定 → 密碼與登入」中為此登入儲存驗證器金鑰後，Somnus 會自動填寫驗證碼。',
    vaultCodeSkip: '略過',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '重新啟動 Somnus 後可使用儲存圖片。',
    restartToSaveImages: '重新啟動 Somnus 以儲存圖片',
  ...
  tips: {
    ...
    items: {
      ...
      skills: {
        title: '教一次就夠',
        text: '技能是一組說明檔，Somnus 會在需要時自行載入。'
      },
      messaging: {
        title: '離開電腦也能用',
      ...
      profiles: {
        title: '設定檔彼此獨立',
        text: '每個都是獨立的 Somnus — 自己的金鑰、記憶與工作階段。'
      },
      'composer-mentions': {
        title: '附件與指令',
