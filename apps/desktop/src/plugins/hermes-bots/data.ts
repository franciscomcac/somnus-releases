 ...
 *  (researchbuddy). Reserved tokens are dropped so a bot renamed "Somnus"
...
export function newBotChat(bot: RosterRow) {
  if (typeof host.newChat !== 'function') {
    host.notify?.({
      kind: 'error',
      message:
        getPluginCtx()?.i18n?.t('bot.openAnotherChatUnsupported') ?? 'Update Somnus to open another Bot chat.'
    })
