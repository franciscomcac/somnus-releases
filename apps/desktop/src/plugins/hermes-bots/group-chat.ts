 ...
 *  clients (Somnus Go, headless backends) see rooms regardless of which
 ...
 *  renamed bot never shows up as its raw profile id or a stale "Somnus"
 *  (community report, Aug 21 2026: renamed default still read "Somnus is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as Somnus (matching displayName and the
 ...
 *  failing `default`s are never one anonymous "Somnus" — judged against the
...
export function groupSpeakerLabel(name?: null | string, group?: null | string) {
  ...
  if (boundary !== -1) {
    ...
    const label = title || (profile.toLowerCase() === 'default' ? 'Somnus' : profile)
  ...
  // title, then the local row's display_name, then default → Somnus.
  ...
  return isDefault ? 'Somnus' : trimmed
}
