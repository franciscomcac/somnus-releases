...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Somnus doesn't know how to remove it. */
...
// non-desktop) Somnus simply carries none of it. See store/profile-share.ts.
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Somnus, 'hub' = installed. */
  provenance?: 'agent' | 'bundled' | 'hub'
}

/** One entry of the built-in optional-skills catalog (optional-skills/ in the
 *  repo) — official skills that ship with Somnus but install on demand. */
 ...
 *  not Somnus); elsewhere, driver health from `cua-driver doctor`. `null`
