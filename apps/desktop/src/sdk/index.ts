...
export interface PluginProfileRoute {
  ...
  profile: string
  /** Backend Somnus profile served by that route. */
...
export const host = {
  ...
  connections: async () => {
    ...
    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update Somnus.')
    }
  ...
  agents: async () => {
    ...
    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update Somnus.')
    }
  ...
  newChat: (profile?: null | string | PluginProfileRoute, options: PluginNewChatOptions = {}): void => {
    if (options.workspaceMode === 'bots') {
      ...
      if (!openTab) {
        notify({ kind: 'error', message: 'Update Somnus to open another Bot chat.' })
  ...
  profileRoutes: async () => {
    ...
    if (!getProfileRoutes) {
      throw new Error('Somnus connection routing unavailable')
    }
  ...
  request: async <T>(method: string, params: Record<string, unknown> = {}, timeoutMs?: number): Promise<T> => {
    ...
    if (!gateway) {
      throw new Error('Somnus gateway unavailable')
    }
...
/** Somnus' reasoning levels, so a plugin surfacing a thinking depth uses the
