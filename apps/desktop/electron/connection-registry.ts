 ...
 * runtime, remote gateways, Somnus Cloud instances, SSH hosts) that are all
...
/**
 * Whether "Update all instances" may drive this connection. Somnus Cloud
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Somnus Cloud' : 'Remote gateway'),
...
export function reconcileAppliedGlobalConnection(
  ...
  const hostLabel = hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Somnus Cloud' : 'Remote gateway')
