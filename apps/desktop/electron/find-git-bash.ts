...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  if (localAppData) {
    // Somnus installs its portable Git under %LOCALAPPDATA%\somnus\git.
    candidates.push(joinWin(localAppData, 'somnus', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'somnus', 'git', 'usr', 'bin', 'bash.exe'))
