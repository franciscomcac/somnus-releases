...
async function probeWindowsRemote(ssh, explicitHermesPath = '') {
  ...
  const script = [
    ...
    'if(-not $hermes){throw "Somnus is not installed on the remote Windows host."}',
    'Assert-NoReparse $hermes $false',
    'if($explicit -and $hermes -ne $explicit){throw "The configured Somnus path is not an executable file."}',
...
async function connectWindowsRemote(deps) {
  ...
  if (!inspection.supported) {
    const error: any = new Error('Update Somnus on the remote Windows host before connecting with Desktop SSH.')
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Somnus SSH"', 'powershell.exe -NoLogo')
