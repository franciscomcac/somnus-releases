 ...
 * "Another Somnus update is already running (PID <itself>, started 1s ago)" —
 ...
 * The lead sentence is plain: nothing changed and Somnus keeps running. The raw
...
export function describeUpdaterHandoffFailure(outcome: Pick<UpdaterHandoffOutcome, 'message'>): string {
  const lead =
    "The updater couldn't start, so nothing was changed and Somnus keeps running as before. " +
