...
if (IS_WINDOWS) {
  ...
  // expose Somnus sessions/config to every packaged app on the machine.
...
// always-on timers. A completely idle, minimized Somnus burned ~20% CPU
...
function resolveHermesHome() {
  // Somnus keeps its own data folder and ignores HERMES_HOME (env or registry):
  // a customer who also runs Somnus must never have Somnus read or write it.
  // SOMNUS_HOME is the explicit override (support / testing).
  if (process.env.SOMNUS_HOME) {
    return normalizeHermesHomeRoot(process.env.SOMNUS_HOME)
  }
  ...
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, SOMNUS.homeDirName)
  }

  return path.join(app.getPath('home'), SOMNUS.homeDirNamePosix)
}
...
// ACTIVE_HERMES_ROOT — the canonical mutable Somnus install. Same path
...
// active-profile.json records which Somnus profile the desktop launches its
...
const APP_NAME = process.env.HERMES_DESKTOP_APP_NAME || SOMNUS.appName
...
if (IS_WINDOWS) {
  app.setAppUserModelId(SOMNUS.appId)
}

// Seed the native About panel with the live Somnus version. This is refreshed
...
function logPoolSpawnFailure(label: string, error: unknown): void {
  ...
  if (isBackgroundSlotWaitTimeout(error)) {
    ...
    rememberLog(
      `Somnus backend for profile ${label} failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
  }
}
...
let bootProgressState = {
  ...
  message: 'Waiting to start Somnus backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    ...
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Somnus will start automatically when it completes…',
        12
      )
    },
    ...
    if (result && result.ok && result.manual) {
      ...
      void dialog
        .showMessageBox({
          type: 'error',
          title: 'Somnus update',
          message: "Somnus couldn't finish updating",
          detail:
  ...
  if (outcome === 'timeout') {
    ...
    await advanceBootProgress('backend.update-restart', 'Restarting Somnus to load the updated app…', 14)
...
async function findSystemPython() {
  ...
  // CPython but several Somnus deps (notably pywinpty's Rust-built
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'somnus', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'somnus', 'git', 'bin', 'git.exe'))
...
async function checkUpdates({ force = false }: { force?: boolean } = {}) {
  ...
  if (!directoryExists(gitDir)) {
    return {
      ...
      message:
        "This copy of Somnus can't update itself from inside the app. Download the latest version from the Somnus website, " +
        `or reinstall Somnus to enable in-app updates. Details: ${updateRoot} has no version-control metadata.`,
...
async function claimBackendChild(child, command, profile, nonce, outputTail: BackendOutputTail | null = null) {
  ...
  if (decision.action === 'fail') {
    ...
    throw new Error(
      `Somnus backend (PID ${child.pid}) died before its identity could be recorded: ${decision.reason}${outputTail?.describe() ?? ''}`
    )
  }
  ...
  if (decision.action === 'degrade') {
    ...
    rememberLog(
      `WARNING: process start marker probe failed for live Somnus backend PID ${child.pid}; ` +
  ...
  try {
    ...
    await waitForBackendExit(child)
    throw new Error(
      `Could not persist ownership for the Somnus backend: ${error.message}${outputTail?.describe() ?? ''}`
    )
  }
}
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Somnus — this window will close and the updater will open. Don’t reopen Somnus yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // Somnus processes running from this venv.  The updater normally refuses
    ...
    if (scriptHandoff) {
      ...
      // abort with "Another Somnus update is already running (PID <itself>)" —
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  // The updater invokes the venv's Somnus launcher, which in turn requires the
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Somnus — this window will close. Don’t reopen Somnus yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
async function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Somnus at ${ACTIVE_HERMES_ROOT}`,
    command,
    args: ['-m', 'hermes_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      hermesHome: HERMES_HOME,
...
async function resolveHermesBackend(backendArgs) {
  ...
  if (overrideRoot && isHermesSourceRoot(overrideRoot)) {
    const backend = await createPythonBackend(overrideRoot, `Somnus source at ${overrideRoot}`, backendArgs)
  ...
  if (!IS_PACKAGED && isHermesSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = await createPythonBackend(SOURCE_REPO_ROOT, `Somnus source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Somnus runtime at ${ACTIVE_HERMES_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  if (!IS_PACKAGED && process.env.HERMES_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    if (hermesOverride) {
      ...
      if (resolvedOverride) {
        ...
        rememberLog(`Ignoring Windows Somnus override under WSL: ${hermesOverride}`)
      }
    } else {
    ...
    if (hermesCommand) {
      if (looksLikeDesktopAppBinary(hermesCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Somnus CLI: ${hermesCommand}`)
        hermesCommand = null
      }
    }
    ...
    if (hermesCommand) {
      ...
      if (
        ...
        return {
          label: `existing Somnus CLI at ${hermesCommand}`,
      ...
      rememberLog(
        `Ignoring existing Somnus CLI at ${hermesCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }
  ...
  //    take ownership.
  const python = IS_PACKAGED ? null : await findSystemPython()
  ...
  return {
    ...
    label: 'Somnus not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function runEnsureRuntime(backend: any, assertStillOwned: () => void): Promise<any> {
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Somnus install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Somnus recovery was handed off to Somnus Setup. The desktop will restart when recovery completes.'
      )
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Somnus install was cancelled.') as any
  ...
  if (!isHermesSourceRoot(ACTIVE_HERMES_ROOT)) {
    throw new Error(missingInstallPartMessage(`Somnus source files are missing or incomplete at ${ACTIVE_HERMES_ROOT}`))
  }

  // On Windows, preflight Git Bash. Somnus' terminal tool calls bash.exe
  ...
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      "Somnus needs a helper called Git for Windows, which isn't installed. " +
        'Choose Repair install to add it automatically, or install it yourself from git-scm.com and reopen Somnus.'
    )
  }
  ...
  backend.label = `Somnus at ${ACTIVE_HERMES_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Somnus runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
    ...
    (requestState: any) =>
      new Promise((resolve, reject) => {
        ...
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
          reject(new Error(`Unsupported Somnus backend URL protocol: ${parsed.protocol}`))
        ...
        req.setTimeout(timeoutMs, () => {
          req.destroy(new Error(`Timed out connecting to Somnus backend after ${timeoutMs}ms`))
        })
...
// signal so the chat composer doesn't stay stuck on "Starting Somnus...".
...
//
// Hosted Somnus gateways gate the dashboard behind an OAuth provider (e.g.
...
// single-shot boot() with no retry) surfaced as the "Somnus couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Somnus
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Somnus Cloud agent…' : 'Sign in to Somnus gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Somnus backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// --- Somnus Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// so a single override flips every Somnus surface to the same portal.
...
// Discover the hosted (Somnus Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Somnus Cloud. Open Settings → Gateway, choose Somnus Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (body === undefined) {
      ...
      if (error && error.statusCode === 401) {
        const err = new Error(
          'Your Somnus Cloud session has expired. Open Settings → Gateway and sign in again.'
        ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Somnus Cloud session has expired. Sign in to Somnus Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Somnus Cloud org tag on cloud-mode entries so Settings can
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  return {
    ...
    remoteUrl,
    // The persisted Somnus Cloud org (slug/id) for a cloud connection, or '' for
...
// `org` (optional) is the Somnus Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Somnus Cloud instance, NOT a user-owned remote gateway,
...
async function buildRemoteConnection(
  ...
  if (!token) {
    throw new Error(
      'Remote Somnus gateway is selected, but no session token is saved. ' +
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Somnus
...
async function testDesktopConnectionConfig(input: any = {}) {
        ...
        try {
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Somnus on the remote host before connecting with Desktop SSH.'
            }
          }
  ...
  // connect to Somnus gateway". Mirror the renderer's connect here so the test
...
async function runPoolBackendStart(
  ...
  rememberLog(`Starting Somnus backend for profile "${profile}" via ${backend.label}`)
  ...
  child.once('error', error => {
    rememberLog(`Somnus backend for profile "${profile}" failed to start: ${error.message}`)
    void teardownFailedLocalBackend(poolKey, entry).catch(cleanupError => {
      rememberLog(
        `Somnus backend for profile "${profile}" cleanup failed: ${cleanupError instanceof Error ? cleanupError.message : String(cleanupError)}`
      )
    })
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(formatBackendExitLine(`Somnus backend for profile "${profile}" exited`, code, signal, outputTail))
    releaseBackendChild(child)

    if (!ready) {
      rejectStart?.(
        new Error(
          `Somnus backend for profile "${profile}" exited before it became ready (${signal || code}).${outputTail.describe()}`
        )
      )
    }
  })
  ...
  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    ...
    label: `Somnus backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Somnus backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
function startAttachedBackendMonitor(attached: AttachedBackend) {
  ...
  attachedBackendMonitor = setInterval(() => {
    void waitForHermes(attached.baseUrl, attached.token, undefined, 'token', {}).catch(() => {
      ...
      scheduleUnexpectedPrimaryRecovery({ error: 'The Somnus backend this app attached to exited.', ready: true })
    })
...
function reportPrimaryRecoveryCrashLoop(code: number | null, signal: string | null): boolean {
  ...
  const message =
    'Somnus backend keeps crashing right after it restarts; not restarting it again. Relaunch Somnus.'
...
async function runHermesStart({ supervisorRecovery = false }: { supervisorRecovery?: boolean } = {}) {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('Somnus is already running in another window.')
  }
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Somnus backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      await advanceBootProgress('backend.remote', `Connecting to remote Somnus backend at ${remote.baseUrl}`, 24)
      await waitForHermes(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Somnus backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Somnus runtime', 28)
    ...
    if (setup.kind === 'attached') {
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Attached to the running Somnus backend',
        progress: 94,
        running: true,
        error: null
      })
    ...
    await advanceBootProgress('backend.spawn', `Starting Somnus backend via ${backend.label}`, 84)
    rememberLog(`Starting Somnus backend via ${backend.label}`)
    ...
    if (!processOwner) {
      ...
      throw new Error('Somnus backend start was superseded by a newer connection attempt.')
    }
    ...
    hermesProcess.once('error', error => {
      ...
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Somnus backend error: ${error.message}`)
        scheduleUnexpectedPrimaryRecovery({ error: error.message, ready: backendReady })
        rejectBackendStart?.(new Error('Somnus backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Somnus backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Somnus backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    hermesProcess.once('exit', (code, signal) => {
      ...
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(formatBackendExitLine('Ignoring stale Somnus backend exit', code, signal, primaryOutputTail))
      ...
      rememberLog(formatBackendExitLine('Somnus backend exited', code, signal, primaryOutputTail))
      ...
      if (!backendReady) {
        const message = `Somnus backend exited before it became ready (${signal || code}).${primaryOutputTail.describe()}`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Somnus backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentHermesLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Somnus backend to launch', 86)
    backendConnectionState.assertCurrentAttempt(connectionAttempt)
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Somnus backend to become ready', 90)
    backendConnectionState.assertCurrentAttempt(connectionAttempt)
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Somnus backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Somnus backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Somnus is
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Somnus app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Somnus icon drops out of cmd/alt-tab — especially when the
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Somnus in the normal
...
// window showing only the composer and its scrollback, so Somnus can be driven
...
// left the user with NO Somnus window after the second toggle (#88513).
...
function createWindow() {
  ...
  installWindowRendererLifecycle(mainWindow, {
    ...
    callbacks: {
      ...
      // reclaim, an external kill). Somnus never does this itself and never reloads it
      ...
      onRendererTerminated: details => {
        ...
        void loadRendererLoadErrorPage(mainWindow, {
          title: 'Somnus desktop UI was terminated',
          errorDescription:
...
// "Starting Somnus…". Before the renderer's backoff loop reconnects, it asks us
...
ipcMain.handle('hermes:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    if (!backend.command) {
      return { ok: false, error: 'Somnus is not installed yet' }
    }
...
// --- Somnus Cloud (cloud-auto-discovery Phase 3) ---
...
// Resolve the canonical Somnus version (the one `release.py` bumps in
// hermes_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Somnus version instead of the Electron app's own package.json version,
...
// Re-resolve the live Somnus version and push it into the native About panel
...
// The About page's "Restart Somnus" button (shown when bundleSwapPending):
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Somnus agent venv at ${VENV_ROOT}.`
    }
  }
...
// hermes://mcp/install?name=NAME&config=B64 — the vendor "Add to Somnus"
...
app.on('before-quit', event => {
  ...
  // quitting Somnus never keeps another app's chord hostage.
