...
const BOOTSTRAP_FAILURE_REMEDY =
  'Common causes: no internet connection, antivirus blocking the installer, or another copy of Somnus running. ' +
  'Close other Somnus windows and choose Reload and retry; if it fails again, open the logs and send them to support.'
...
export function describeBootstrapFailure(failedStage: string | null | undefined, rawError: unknown): string {
  ...
  const lead = label
    ? `Setting up Somnus stopped during the '${label}' step.`
    : 'Setting up Somnus stopped before it could finish.'
...
/**
 * Error.message for an installed Somnus with a piece missing (source tree,
...
export function missingInstallPartMessage(whatIsMissing: string): string {
  return (
    "Part of Somnus' installation is missing (it may have been deleted or quarantined by antivirus). " +
