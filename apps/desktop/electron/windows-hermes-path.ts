 ...
 *      and Somnus launcher. Marker-only or partial runtimes must use --repair;
 ...
 * venv Python interpreter and the Somnus launcher that drives `hermes update`.
...
export async function resolveVenvHermesCommand(
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Somnus at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Somnus Python at ${python}`,
    command: python,
