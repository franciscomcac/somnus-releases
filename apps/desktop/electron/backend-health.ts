...
export const REMOTE_UNSIGNED_OAUTH_MESSAGE =
  'Remote Somnus gateway uses OAuth, but you are not signed in. ' +
...
/**
 * True when the backend URL points at a Nous-managed Somnus Cloud instance
...
export async function waitForHermesReady(baseUrl: string, options: HermesReadyOptions): Promise<void> {
  ...
  throw new Error(`Somnus backend did not become ready: ${detail}`)
}
