 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Somnus dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Somnus install on the remote (login-shell probe),
...
// e.g. "Somnus v0.18.2 ..."), or '' on failure. Surfaces WHICH hermes a
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Somnus SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
async function probeRemoteHermesHome(ssh) {
  try {
    ...
    const error: any = new Error('Could not resolve the remote Somnus home.')
 ...
 * bytes; it never imports or executes code from the changing Somnus checkout.
...
async function assertRemoteInstallUpdateClear(ssh, hermesHome) {
  ...
  try {
    ...
    const error: any = new Error('Could not prove that the remote Somnus install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ? `Remote Somnus update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote Somnus update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemoteHermesProfiles(ssh) {
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Somnus profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
async function readRemoteInstallId(ssh) {
  ...
  try {
    ...
    const error: any = new Error('Could not read the remote Somnus install id.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Somnus home.')
...
// execing Somnus. mutexPath is expandRemotePath() output — a complete shell
...
async function spawnRemoteDashboard(
  ...
  if (!(await remoteSupportsSshOwnership(ssh, hermesPath))) {
    const err: any = new Error(
      'The remote Somnus install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Somnus on the remote host to continue using Desktop SSH mode.'
    )
...
async function connect(deps) {
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this Somnus build (${lock.reason}). ` +
